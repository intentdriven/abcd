...
- **A hook runs only the `abcd` your install recorded.** GHSA-gx3m-3224-qqcv (CWE-426): the four hook shims that carry a PATH rung accept a PATH-resolved binary only where `~/.abcd/path-entry` records that exact path, and only after the existing absolute-path, outside-the-working-tree and not-world-writable checks. A copy installed on PATH by an earlier one-liner carries no such record and stops being accepted until an install writes one, so re-run the install one-liner or `abcd ahoy install` on any machine that relies on that rung. A binary the record does not name gets a loud one-line refusal naming it and the reason: for PreToolUse that is an UNGUARDED line and exit 1, with the command still running unguarded, and never the exit 0 a harness reads as approval. SessionStart carries no PATH rung and still fails closed. (iss-2609012039107700)
- **A configuration root owned by another user is refused.** Where git will not answer, the rules-root fallback refuses a repository-shaped marker directory whose owner is not the caller: it fails closed on the working directory with the bundled rules and the bundled hazard registry, and names the remedy. The opt-in is the home-scoped `~/.abcd/trusted-roots`, read only when it is a regular file this uid owns that no one else can write, so the tree under judgement can never assert its own trust. A container bind mount, a shared CI checkout or any other foreign-uid checkout needs that declaration before its `.abcd/rules.json` and `.abcd/guard.json` are read. (iss-2609020259564193)
- **`abcd spec close` takes `--impact`, and refuses rather than shipping an intent that declares none.** The flag stamps `additive`, `breaking` or `fix` on an intent carrying no judgement, validates one the record already carries at the bar the seed applies, and refuses a flag that disagrees with a recorded judgement. An intent produced entirely by abcd's own verbs therefore satisfies abcd's own record-lint, and a close with no impact on either side stops at the verb instead of at the record gate. (iss-126)
- **The comparative reading can be commissioned.** `abcd reading assemble --position comparative --target <commit>` derives from the record the one committed widening run at that target whose items carry no disposition and no admission, and hands the reading that run's returned configurations, projected to the two fields the widening body carries, together with the six declared selection criteria. No operand names the run: the candidate set is a fact about the record rather than a choice made at the prompt. (itd-2609020625407419)
- **A reading is commissioned with two things: where it stands and what it reads against.** `abcd reading assemble --position <position> --target <commit>` is the whole invocation, and what the reading is handed comes from the committed preset entry for that position, applied with no flag. The `--scope` operand and the override stamp are withdrawn: changing what a position reads is a commit to the preset file, reviewed and inside the dirty gate. The manifest records the preset entry applied and its hash, so a run is reproducible from the commit it names. (itd-2609021003095168)
- **`abcd decide "<title>"` mints a decision record.** The id is `adr-<timestamp>` from the same coordination-free allocator issues and intents use, and the skeleton lands under `.abcd/development/decisions/adrs/`, so two branches deciding on the same day cannot both reach for the next ordinal. One derivation serves every reader of an ADR id or filename, and the hand-numbered ordinals 0001 to 0058 and the minted stamps resolve, lint, sort and dispatch side by side, ordinals ahead of stamps in the index order. (iss-2609012206053814)
- **The brief's glossary fixes one spelling per sense.** Eight core entries name every sense of the record's colliding terms: phase, plan, roadmap, record, surface, ledger, construal, reading position and loop, each with a repo-relative link to the chapter it lives in, and the roadmap phases page and the intent surface page point at the entry for the word they use. The ledger's own bounded context lands beside them with nine terms, so the vocabulary a reading is handed contains the words its construal is written in. (iss-2609012245352480, iss-2609012259586524)
- The rendered site publishes the glossary as its own page set at `/record/glossary/`, one page per entry in the explorer's navigation, and the first use of each entry on a record or entry page links to it, aliases included, case-insensitive and whole-word. A use inside a code span, a heading, an existing link, or on the term's own page is left as the record wrote it, dedup is by entry rather than by spelling so a term's aliases together earn one link, and a repository keeping no glossary gets no pages, no navigation entry and no links. (iss-2609020922150364)
- The cold-reading exclusion floor carries an entry for the local ledger tier `.abcd/.work.local`, so every manifest asserts that the tier was refused and the assembler enforces it: a reader checking a manifest can see that framing traces and declined construals were kept out, rather than inferring it from silence. The core assembler version moves to 1.3.0, an added floor entry being an added promise rather than a rewording, and the charter's rendered table carries the row. (iss-2608311238236490)

### Fixed

- **A secret inside an archive is a finding.** The payload scan decodes zip, gzip, bzip2, zlib, tar and PNG's compressed chunks through the standard library alone, classifies every file by magic bytes rather than by name, and scans the decoded regions under the same byte rules, so a token in a gzip member, a zip entry or a DEFLATEd chunk is reported and a zip renamed `.png` is labelled and decoded as a zip. Decoding draws on one budget per payload file: 4 MiB of output, four nesting levels and 1024 operations, and a container that would exceed any of the three is refused rather than decoded, so a decompression bomb cannot exhaust the host. The formats abcd still cannot read, xz, 7z and zstd, which have no standard-library decoder, and JPEG entropy streams, PDF FlateDecode and any unrecognised magic, stay named as content-unverified rather than reported clean, and the report carries the decoded tier, the unverified tier and the reason per unverified path separately. (iss-2608291832160371)
- A truncated or narrow-rewrapped private key is redacted again. A single-line PEM body of sixteen to thirty-nine characters opens a block once more, on two grounds that width says nothing about and that both have to hold: the header was pasted, its marker being the whole content of its line rather than named inside a sentence that runs on past it, and the run carries a byte of the base64 alphabet outside the letters. A key re-wrapped to a narrow column by a mail client, or cut short by a log rotation, therefore stops being recorded as handled while a fragment of its body stands in the text. A real body of that width is still missed where its header carries prose after the marker or its run happens to be letters only. (iss-2609090932441377)
- **A memory page's filename cannot carry a credential.** The store redactor judges the page name at the write boundary and refuses it, never rewrites it, at a hard-fail bar, so a credential-shaped slug reaches neither the file, `index.md`, `log.md` nor the registry back-link. The judge reads the joined name, the three components the page-name pattern parses out of it, and every suffix beginning immediately after an underscore, so a token whose own prefix straddles a separator, `topic_ghp_` followed by its body or a Stripe key splitting into domain and slug, is caught instead of hiding from both passes at once. (iss-2609020321100138, iss-2609090953184203)
- **`abcd ideate record` redacts the verdict before it writes it, and refuses on a degraded scanner.** The idea text, each claim and its primary source, each grill note, each kill attempt and each rejected alternative go through the shared scanner on the way into the validated document, ahead of the terminal-safety cleaning and well ahead of the renderer, with the literal-`$HOME` backstop the history and memory stores apply on the same boundary. The rendered record and the composed decision-log pointer line are re-scanned together before either is written and any blocking residual refuses the run, which is intent's fail-closed stance rather than capture's redact-and-report one, because the verb writes into the same committed tier: `.abcd/development/`. The record write is a one-shot exclusive create, so a refusal leaves no partial document behind, and the result reports how many redactions were made, so a composer learns they have just pasted a credential. (iss-2608291817368607, iss-2609020127281995)
- **Every agent-supplied string bound for a committed record is redacted before it is neutralised.** The intent-audit ingest routes a verdict's per-criterion rationales, gap-audit claims, scope-condition rationales and narrowings, evidence refs and quotes, the dead-letter reason, and the verifier id and version and an attestation's kind and ref through the redactor, so a home path in an attestation ref or a hostname in a verifier id stops landing verbatim in the shipped intent's Audit Notes. What has a declared shape is validated instead: `policy.rubric_hash`, `policy.prompt_hash` and a present attestation digest must read as `sha256:` plus 64 lower-case hex, and a payload that fails quarantines. A reading ingest redacts every payload-derived string bound for `run.json` or `refusal.json`, including a refused item's criterion and candidate id, the unknown and reserved field names, the out-of-vocabulary value, the decode error, the regime and instrument mismatches and the recorded instrument identity. (iss-2608300924205748, iss-2609022002241168)
- An interactive install cannot switch the deep secret scan off by typo. The `scan_deep` slot re-checks the answer against its choice set and takes the partial-install path on anything outside it, exactly as the `docs_target` and `oracle_backend` slots beside it do, so answering `yes`, `y` or `TRUE` to a question about scanning for secrets stops resolving to a disabled scanner with no diagnostic. An explicit `false` still disables the deep scan deliberately, and the choice set and its default are declared once for the collect, override and would-change paths to read. (iss-2609090642031172)
- **A wired SessionEnd hook stores a transcript on a machine where `ahoy install` never ran.** The transcript store is user-level, self-creating and keyed on the repository's root-commit SHA at `~/.abcd/transcripts/<root-sha>/`, so capture carries no install precondition and the hook stores a transcript instead of logging a line and exiting 0. A per-repo copy is an opt-in pull declared in `~/.abcd/local-transcript-roots`, home-scoped, uid-owned and not other-writable, landing in the gitignored `.abcd/.work.local/transcripts/`. A corpus at `~/.abcd/history/<root-sha>/transcripts/` is moved into the store on first resolve, reported on stderr, and tombstoned at the old path. (iss-95)
- **Every `capture` verb addresses the checkout's ledger, wherever it is run from.** Each front door resolves the checkout root through git's toplevel before it builds a request, and refuses both remaining states rather than guessing: a repo-shaped tree git will not answer for, and no repository at all. A capture from a subdirectory therefore stops minting a second ledger under it and reporting success with a path nobody reads as unusual, a status render from a package directory stops reporting an empty board, and a run outside a checkout writes nothing rather than creating a full ledger skeleton in a plain directory. A ledger already sitting below the checkout root is named on stderr and left untouched, and the bare board stays read-only. (iss-2609090951291524)
- A committed `.abcd/record-lint.json` cannot walk the record gate outside the checkout. Containment is checked once at config load, through the canonical relative-path validator, refusing every configured repo-relative value that is absolute, unclean, backslashed or climbs out with `..`: the top-level roots list, each rule's sixteen path fields, every `record_stores` key the config writes, and each index-drift doc and directory pair. A gate at the config door cannot be forgotten by a field added later. (iss-2608301308367566)
- **A frontmatter value means what YAML says it means, for every gate and every store.** The one same-line scanner strips a trailing comment before it hands a value on, a comment starting at a hash preceded by whitespace and outside quotes, so a hash inside a quoted value stays part of the value while `grounds: {} # todo` and `severity: minor # todo` read as the value they spell rather than as the raw remainder of the line. Emptiness is decided by the class of node a value spells, null, empty flow collection or empty string, after reading off its tag, anchor and alias properties, so `!!null null` and `!<tag:yaml.org,2002:null>` are the same absent value as `!!null` and a blank required field cannot walk past `record_schema` by finding a spelling. No YAML dependency was added. (iss-2608301744268001, iss-2608301808198621)
- **A construction the cold-reading exclusion floor cannot resolve is refused at admission and named, rather than passed through unscanned.** Refused: a delimited block that does not open at line 0 but opens one to a reader of the bundle, because only blank lines, whitespace, a byte-order mark or an HTML comment precede it; an explicit key in a flow mapping, whatever the key is named; a fence delimiter inside the frontmatter block; and an attribute value that opens on the line after its equals sign. The fence mask is scoped to the body and computed after the block is located, so nothing inside the frontmatter can switch the excluded-key scan off. A raw heading is bounded by a CRLF blank line as by an LF one, and an opener that reaches the end of the document with neither a hard nor a soft bound is refused as a heading element that is never closed, so its title is not read over the rest of the file. (iss-2608301237456350, iss-2608301251398360, iss-2608301350533102, iss-2608301350534164, iss-2608301421380392)
- **A reading's manifest and its bundle describe one set, item by item.** Every include row declares whether the exclusion floor parses what the row admits, and the redaction pass runs over exactly the rows declared parsed, so admission and examination are one declaration instead of a row on one side and a file-extension test on the other. An item from an unparsed row, a source or test file a committed preset opts in, travels whole and is marked `unscanned` in the manifest, and the manifest's key and heading exclusions are asserted per item and only for the items the floor parsed. A record-shaped document inside a Go fixture therefore arrives disclosed rather than silently, and a scan that ran and a scan that never ran stop producing the same artefact. (itd-194, iss-2608301450065320)
- **`abcd reading assemble` runs over this repository's own corpus at every assembling position.** The 21 glossary records open their frontmatter delimiter at line 0, the `mattpocock/skills` attribution comment having moved verbatim to the line after the block closes, so the displaced-block refusal has nothing to refuse there and record-lint gains no finding. The markup-shape scan reads the unfenced body, with fenced lines blanked rather than dropped so a match offset still counts the document's own newlines, so an admitted document holding a fenced HTML example assembles while the same shape outside a fence still refuses and names itself. (iss-2609021457186209, iss-2609021954509554)
- **A reading's assembled input fits the reader it was calibrated for, and each position reads its own object.** The committed preset file holds one entry per position stating three things: the object set the run is about, the kinds admitted within it, and the estimated-token window the entry was calibrated for, beside the figure measured and the commit measured on. An eval dry-run-assembles every assembling position's entry on a clean clone of HEAD and fails past the declaration, naming the position, the measured figure and the declaration, so an assembly of some ten megabytes at every position, which no reader can be handed, is caught by the cold-reading lane rather than by the reader. The three assembling positions' item sets are pinned by digest and asserted distinct, and two assemblies of one entry produce byte-identical bundles, so what a position reads moves by a commit to the preset file and by nothing else. (itd-2609020625400445, iss-2608311501186646, iss-2608311501240566)
- **Each reading position is handed the object its definition names.** The include table admits the brief's surfaces, internals, delivery and meta chapters beside its product and constraints ones, so the object is what both design documents call the brief's current text, with the evidence chapter excluded on the stated ground that a prior verdict is revision history. The shipped intents leave the widening position, which neither design document lists in its object, and the floor gains a widening-scoped entry so the manifest asserts the withdrawal. Drafts and planned intents travel at entailment only where the committed entry's object set names them, the companion's admissibility surviving as an `admit_drafts_and_planned` switch that is off by default and refused at load on any other position. (iss-2609012259587904, iss-2609021748300398, iss-2609021153264023)
- A comparative reading can follow the widening ingest that supplies its candidates. The derivation reads the decision record's "at the target" as reaching an ancestor: a committed widening run qualifies when its recorded target equals the target, or is an ancestor of it and every path changed between the two lies inside the durable readings store and the issue ledger's own record families. A run whose target is not an ancestor is not listed, and a run across which anything else changed is listed and refused, naming the first such path. The comparative manifest carries `candidate_run_target` beside `target_commit`, so a reader holds both commits and can diff them, and committing the widening run's records, the act the design sequences between an ingest and the next reading, no longer disqualifies the run. (iss-2609021833302981)
- The comparative assembly refuses whenever a fate record differs between the working tree and the commit its manifest names. Any dirty path under the ledger's dispositions or admissions family counts as included at that position, both roots derived from the ledger's own constants rather than named by hand, so a committed disposition deleted in the working tree alone, or an uncommitted admission that disqualifies one of two otherwise-ambiguous runs, is named in a refusal instead of silently selecting a different candidate run. (iss-2609021950351082)
- `abcd reading ingest` commits a run that returned no items, at every position, so the framework's clean-run contingency is recordable: a null result is a run with an empty item set, written as `[]`. A payload whose every item was refused stays a list-level refusal, because those are different facts and recording the second as the first would lose it. (iss-2609021153269181)
- A reading ingest's refusal names only stages that still stand. The rollback subtracts the stage it clears from the result's `pending_stages`, so a refused run whose own earlier attempt left a stage stops reporting that stage as pending after clearing it. (iss-2609020848468450)
- The `reading ingest` help describes what the verb does, at the terminal and on the generated reference page alike. A refusal after the run is proven writes its refusal record and makes exactly one delete, on its own run id; nothing durable is written before that point, the stage root and its lock being created first; the orphan sweep is given both its outcomes, the run's reading records leaving the committed ledger where there is no commit marker and the run standing with only the stage going where there is one; and the reserved-name rule points at the table the gate reads, one row per regime, the generative regime carrying none. Each sentence is now held to the behaviour by a test. (iss-2609020843067458)
- A `reading` refusal carries one `reading:` tag rather than two. Every sub-verb composes its refusal from the verb name plus the core message with the core's own prefix trimmed, so the assemble verb and the bare verb stop rendering `abcd: reading: reading: ...` on a core refusal, as the ingest verb already did. (iss-2608311145286014)
- `commands/reading.md` describes the bare verb's `definitions` field as what the definition locator resolved rather than as what is present under `agents/`, and says that a definition silent about its position or its regime, or stating another position's, refuses the whole verb with exit 2 instead of being listed. An operator reading a short list therefore reads a repository with fewer definitions, not a malformed one being skipped. (iss-2608311039586922)
- The four cold-reading definitions state the ordering rule the readings companion states: items are returned in the order they arise in the object. The sentence is byte-identical across the four blindness cores, and their prompt versions move together. (iss-2609021153261145)
- The entailment size report states the yield bound beside the figures: how many projected intents carried a `## Mechanism` section, how many carried the `None stated.` nullity, and how many carried neither, counted per file from the candidates by path and field. The proportion the position's causal side depends on is therefore on the record for the run rather than discovered by a reader afterwards. No shipped intent is backfilled. (iss-2609012259585189)
- A reading's contribution is traceable from both ends. An intent draft promoted from an accepted reading item carries `origin: contributed-by-reading <rdg-N>/<rdi-N>`, naming the run and the item, and the item carries `promoted_to` pointing forward, so from a reading item the record shows what it caused and from an intent whether a reading occasioned it. Promotion of an issue keeps saying `extracted-from-record`, because an issue is something a person noticed and a reading item is something an instrument returned. (itd-2609020625400169)
- Committing a reading run's records keeps the regime operator-surface guard green. The walk skips the readings record family, derived from the ledger's own constant, because a family a verb writes from the position's definition is not a channel an operator can set, while a regime key under `.abcd/config/` is still reported. (iss-2609022252265901)
- **Every install route records the PATH entry the hooks check.** `abcd ahoy install` stamps `~/.abcd/path-entry` for the pinned symlink and the `--dev` shim as well as for the owned copy, under the path spelling `command -v abcd` yields, and the three install one-liners write `path=` and `binary_sha256=` too. An install the board calls healthy is therefore one the hooks accept, rather than one whose every hook prints a refusal and takes its degraded route with no rules loader, no shell guard and no transcript capture. A registered copy is refreshed by the bootstrap on the next release and `abcd update` can vouch for it from that record; an owned copy the record does not name is reported as a required, resolvable gap, so a machine an earlier release produced can heal itself, and uninstall drops the record for the entry it names and only that one. (iss-2609091126475539, iss-2609012111159045)
- **`abcd update` can replace the running binary after its release is deleted from the forge.** Ownership is proved from the running executable's own identity, or from the `~/.abcd/path-entry` install record, rather than from a published `checksums.txt` that may have been pruned, so a plugin-provisioned or older PATH install stops refusing as an unprovenanced file. The refusal that remains reinstalls over the file instead of directing the user to remove the only tool that could reinstall it. (iss-2609012000222546)
- `<plugin-root>/abcd ahoy install` run from a terminal lands the owned copy the documentation promises. The bootstrap stamps the data directory it provisioned the root from into `<plugin-root>/.data-dir`, and the resolver falls back to that stamp, validated as an existing absolute directory, when `CLAUDE_PLUGIN_DATA` is unset, which it is in a shell. Following the documented instruction therefore stops degrading to the legacy symlink that dangles after the next plugin update. The artefact is still re-hashed against its recorded `binary_sha256` before any copy, and the degradation notice names every source tried. (iss-2609012111168716)
- The bootstrap says what it declined to do. The migration seed prints a notice when it refuses a mismatched root binary, and the PATH-copy refresh names the copy it left untouched, in tilde form, and why, so a copy that stops being refreshed is visible at the moment it happens rather than when `abcd update` or `abcd ahoy` later calls it foreign. (iss-2609012111160075)
- An inlined SVG carries only attributes on a closed allowlist, built from the committed drawings plus the standard presentation and geometry set, so `style`, `class`, foreign namespaces and anything unweighed by name are refused by the site build. Inlined bytes are concatenated verbatim into the published page, where inline CSS on the root element beat the page's own rules and a child could position itself to the viewport. (iss-2609081940398387)
- **A site manifest cannot point the composer at a file it was never meant to publish.** `home.hero.page` and `home.chapters[].page` are closed to `docs/` and `site-src/`, every quote-type path field is held to a closed set declared for that field, and every manifest path field refuses the git directory through the shared gate. The contributors policy file is the one that renders whole: for any part other than the first bullet it publishes the entire matched section of the file it names, so a manifest aimed at a gitignored scratch file with a matching heading published it verbatim. The committed manifest still loads and the site still renders. (iss-2609081940475662, iss-2609090951279243)
- A human author whose name is a single token stays in the humans row on `/contributors/`. The bots row demotes on a structural machine signal, a `[bot]` name suffix or a vendor-name match together with a machine mailbox, rather than on a vendor token that happens to equal a git author name, and the by-model chart tallies only trailer-shaped values, clipped to the attribution grammar, so a free-form second `Assisted-by` line is not published as a bar label. (iss-2609081940550352)
- **The homepage features an intent whose audit really is met.** The rollup reader takes the first `Audit Notes` section only and refuses a document carrying two, skips fenced lines inside it, and refuses a negative count, so a fenced `NOT_MET -1`, a fenced passing rollup and a duplicate section titled the same all stop lifting a record whose acceptance is not met. The real tree's featured set is unchanged at 26 of 41 shipped intents. (iss-2609081941073604, iss-2609090951277880)
- **The version stamped on a featured record is the release that credits it.** The changelog walk compares whole handles rather than substrings, so a newer section naming `itd-1990` cannot credit `itd-199`; a handle glued to a hyphen on either side, as in a branch name such as `fix/itd-199-cleanup`, credits nothing; and the walk tracks fences ahead of the dated-heading test, so a handle inside a fenced example is not a credit and a fenced dated heading moves no version cursor. Newest-section-first is unchanged, and the committed changelog's own credits are corrected where a substring had stamped the wrong release. (iss-2609081941074556, iss-2609090951280114, iss-2609090951287232)
- **One declaration governs which Go toolchain every lane runs.** Every `setup-go` step in the four shipped workflows, and all three in the release scaffold template, resolve the version from `go.mod` instead of a literal pin, so bumping the go directive moves every job and the format gate together with no workflow edit. The release verify job invokes the same `make fmt-check` the CI workflow does, and the scaffold template, which has an adopter's repository and no Makefile of ours to call, runs the gofmt from the GOROOT the module's declared toolchain resolves to, so a cut is never judged by whichever gofmt the runner's PATH happened to hold. (iss-2609090951291799, iss-2609091128354325)
- A release cut's multi-line refusal renders as the lines it was written as. The reason goes through the block sanitiser, which neutralises a control sequence while leaving the line structure intact, so the surface guard's list of what narrowed since the anchor tag, the stale-intent refusal, and the findings gate's enumeration of the records blocking the cut each print as a heading and one indented line per record instead of one run-on line with question marks where the breaks were. The JSON envelope carried the reason correctly throughout, which is why only the terminal render was affected. (iss-2609091046297503)
- A store's directory chain is proved level by level as it is created. One primitive in `internal/fsutil` serves the transcript, lifeboat and intent stores, with the mode as a parameter, so the private stores keep `0o700` and the record store `0o755` and no directory on disk changes. The intent variant's weaker guarantee goes with it: it checked the leaf for a symlink and then created the chain with a call that follows a symlinked ancestor, a hole its own comment recorded, invisible at its five call sites. (iss-2609091128479544)
