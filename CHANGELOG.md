...
- **One check, and modes are flags (breaking).** `abcd lint` is the one check and runs every target; `abcd lint docs`, `lint outbound`, `lint site` and `lint identity` run one each. `ahoy`'s modes are the flags `--dry-run`, `--identity` and `--remote`, `abcd --version` prints the version report, and `abcd update --check` runs the release check. What stops working: `abcd docs lint`, `abcd site check`, bare `abcd identity`, `abcd ahoy dry-run`, `ahoy identity-check`, bare `ahoy remote`, `abcd version` and `version --check` now exit 2 naming their successor, and `abcd intent new` is an unknown sub-command (file a draft with `abcd intent "<text>"`). Update scripts, hooks and CI steps to the new spellings; `docs cite` and `site build` are unchanged. (itd-2609212130136102)
- **`abcd --help` reads as a map.** The person's verbs render under five labelled groups (set-up, records, checks, portability, release), and `abcd --help --agent` adds a second block listing the verbs agents and hosts call. Every verb runs the same whichever block lists it, and a test fails when a visible verb declares no group. (itd-146)
- **Every verb's help opens with one sentence** saying what it does, what it writes and when it refuses, identical on the command list, the verb's `--help` and its plugin page; a test walks every visible verb and fails on a missing clause or a difference between the three places. (itd-2609212113220149)
- **`abcd launch ship` runs the full pre-flight gate suite, fail-closed.** On top of the secret and PII scan it adds the identity layer (home-directory paths, real emails, GitHub usernames), marker-block sanity, `plugin.json` and `marketplace.json` validation, a dirty-tree refusal, and the warn-fail documentation and hook-compliance checks, so a finding blocks the publish rather than only appearing in the preview. (itd-65)
- **`abcd launch` renders the exact public payload.** The render applies a default-deny filter and proves `.abcd/` never rides along, diffs the payload against the previously published release (`--baseline <tag>`, or `--fetch-baseline` to read the tag's published archive, verified against its checksums), and smoke-tests every shipped command, skill and hook from the rendered snapshot. (itd-66)
- **`abcd launch scaffold` lands a changelog-driven release gate in a managed repository.** Rolling the changelog into a dated version in a reviewed pull request is the release decision, the gate is armed against the reviewed content commit, and a `workflow_dispatch` rehearsal arms the full gate against a simulated release and publishes nothing, so the first public release does not fail on its own gate. (itd-93)
- **A user-scope rules layer.** Conventions you share across repositories live once in `~/.abcd/rules.json` and inject into every repository abcd manages on that machine, layered between the bundled default domains and each repository's own `.abcd/rules.json`, which still wins. A machine with no user rules file loads the bundled and repository layers alone. (itd-117)
- **abcd's own test lanes check the machine's load first.** `make preflight` (and so the pre-push hook) and the eval harness warn before running when a stray process has used nearly all the CPU it could get (its fair share of the machine as loaded, never more than one core) for over the stray limit (30 minutes by default), or the load is far beyond the cores: your own strays by name, pid, age and CPU share with how to stop them, other accounts' only as a count and a total share. The run carries on; inside an autonomous run the warning also lands in the run log. Limits are per machine, in a settings file under `~/.abcd/`. (itd-2609231434459890)
- **A bare `abcd` on an interactive terminal opens with a banner:** the a-b-c-d signal-flag hoist in half-block colour, the version beside it and the tagline beneath, above the unchanged status board. Piped output, CI logs and hook context carry no banner and no escape byte, a source build says `abcd (dev build)`, and `--no-color` renders it without colour. (itd-112)
- **`abcd intent condition` dispositions a scope condition from a reading run.** `abcd intent condition <itd-N> <cond-id> --disposition <survived|narrowed|falsified|untested> --occasioned-by <rdi-N|itd-N> --grounds "<why>"` writes a dated disposition against a shipped intent, keyed to the condition's stamped identity and joined to the reading item or shipped intent that occasioned it, with the narrowing stated where the value is narrowed. It refuses a value outside the enum, an occasion that does not resolve, thin grounds, and an intent not in `shipped/`. (itd-2609020625405251)
- **Every surface chapter carries a sub-verb table**, naming each sub-verb's bucket (lint, review, audit or gate) and whether it exists, and `surface_coverage` checks every row against the binary's command tree both ways: a sub-verb that ships without a row, or a row claiming an unregistered sub-verb, fails the build. (itd-122)
- **The shell guard blocks a kill by pattern or name.** `pkill` with a pattern (including `pkill -f`) and `killall` with a name block under `pkill-by-pattern` and `killall-by-name`, naming the way past: kill the pid you recorded or your own process group. `pkill -g` and `pkill -P` stay allowed through a new `min_operands` pattern field. A kill whose pid list comes from a pattern search inside a substitution is not covered. (iss-2609240646538696)
- **The shell guard expands brace groups instead of refusing them.** `mkdir -p foo/{a,b}`, `cp x{,.bak}` and `rm -rf dir{1..9}` are checked word by word: an everyday group is allowed, a group that expands to a hazard blocks under that hazard's entry, and a group producing more than 4096 words or 1 MiB per command line is refused. (iss-2608282026038930)
- **The shell guard warns on a bare `git stash` in a clone with several worktrees**, where the stash stack is shared: a stash or push without a message, and a pop or apply without an entry, warn under `git-stash-shared-stack` only when `git worktree list` reports more than one non-bare worktree. (iss-2609190338340796)

### Fixed

- The shell guard matches an abbreviated git long option: a unique prefix of `--no-verify`, `--force-with-lease` or `--force-if-includes`, which git runs as the full flag, now blocks as the full flag does. (iss-2609251640354925)
- The no-verify blockers catch the same bypass spelled as configuration: a `git commit` or `git push` that points `core.hooksPath` elsewhere through `-c`, `--config-env` or the `GIT_CONFIG` environment now blocks. (iss-2609251640464212)
- A shell reading its script from a pipe, a here-document, a here-string, `/dev/stdin` or `/dev/fd/0` behind a pipe, or a process substitution (for a shell or `source`) is refused under `interpreter-reads-stream`, so a blocked command piped as text into `sh` or `bash -s` no longer runs unchecked. (iss-2609251640462464, iss-2609252020505990)
- A blocker flag written after a command substitution or a process substitution is still read as the enclosing command's argument: `cd s && rm $(true) -rf *` and `git push >(cat) --force origin main` now block. (iss-148, iss-2608221126066631)
- The shell guard follows a command substitution inside double quotes, the idiomatic spelling (`echo "$(gh repo delete owner/repo)"` now blocks), eight levels deep; a flag glued beside an empty quoted substitution is read as the flag; past the depth, or where the quoted text does not tokenize, the command blocks under `substitution-unread`; and a double-quoted `${…}` holding quotes of its own parses as bash parses it instead of blocking as unparsable. (iss-2609251144159533, iss-2609251640353993, iss-2609251640353405, iss-2609252214217550)
- An unquoted here-document body is read as bash expands it: each command substitution in it (dollar-paren, backtick, or inside an arithmetic expansion) is checked as a command of the redirecting chain, a line ending in an odd number of backslashes joins the next before the delimiter compare, and inside backticks bash's backslash pre-pass is applied before the text is followed. (iss-2609252120204766, iss-2609252214137586, iss-2609260115287911)
- A fixed-output `cat` of a literal here-document is read as its document's text: as the payload of `sh -c`, `bash -c`, `eval`, `su -c` or `env -S`, as the words bash splits it into in command position, in its backtick spelling, and glued to other text; a payload nested past the two execute-a-string layers the guard follows is blocked, and a line that assigns `IFS` before an unquoted fixed output is refused under `ifs-split-unread`. (iss-2609252214215409, iss-2609252305404421, iss-2609252310310823, iss-2609260115380561, iss-2609260115387303)
- ANSI-C strings are read as bash reads them: the string ends at its first decoded NUL, so the guard's internal substitution mark cannot be forged, and the closing quote is found before escapes are decoded, so a string ending in `\c` no longer swallows it. A line the guard cannot split now blocks as `command-unparsable` instead of failing open in the hook. (iss-2609252020432185, iss-2609252120212508)
- An option word the guard cannot know (a dash glued to a command substitution, or a substitution inside a `${…}` default) is read every way it could run, as a stand-alone flag, a flag taking a value, and a shell's `-c` or a verb's payload flag, so a hazard behind `sudo`, `env`, `xargs`, `su -c` or `git` through such a word is blocked; behind `timeout`, a dash-glued word with no duration after it (`timeout --$(…) pkill …`) is warned as an unrecognised launcher rather than blocked. (iss-2609252020507464, iss-2609252120211621)
- The kill-by-pattern and kill-by-name blockers fire when the pattern arrives through a command substitution, and a block that fires only because a program's name is a substitution is reported as `program-name-unknown` with its way past (spell the program's name) rather than under an unrelated entry's lesson. (iss-2609251640353017, iss-2609252120212639)
- The rm-after-cd blocker treats `pushd` and `popd` as directory changes, so a recursive forced delete chained after either blocks as it does after `cd`. (iss-2609251640464735)
- A git alias declared inside a bang alias's body is now resolved and its rewrite checked, two levels deep; an alias nested past that budget blocks under `git-config-rewrite-unread`. (iss-2609020348038749)
- An uncommitted edit to `.abcd/guard.json` that switches the guard off or changes a blocker's tier or pattern no longer takes effect: the committed registry stays in force until the edit is in HEAD, the hook says so, and `abcd guard check` exits 2. Where git cannot confirm HEAD, the edit is refused. (iss-147)
- The `local_username` detector no longer reports a generic account name (a built-in list of CI, image and role accounts, or a one- or two-letter name) as a bare word, so `abcd launch --dry-run` stops hard-failing a clean tree on documented flag text and captures keep an ordinary word that happens to be the account name. The name is still reported where an account stands: after a home root or a tilde, and in an address or login. (iss-236, iss-2609061504302157)
- A login in a URL's userinfo (`https://LOGIN@host/`, with or without a password) is reported as `local_username` rather than suppressed with the rest of the URL. (iss-2609251549447970)
- `home_path_other` catches another person's home path nested under a longer absolute path (`/Volumes/Backup/Users/<name>/x`), inside a `file://` URL, and in any letter case (`/USERS/…`, `/Home/…`), so memory, history and capture redact it. (iss-2608291915432717, iss-324, iss-2609251544556874)
- Under `HOME=/root` (containers, CI as root), a `/root` segment deeper inside a URL path is no longer rewritten or reported as your home directory; only a home at the URL's path root is swept. (iss-2608292005445725)
- Private-key redaction covers more renderings of a PEM body: log-prefixed, CSV, per-line XML, trailing-comment and source-concatenation lines, one-line JSON arrays (also inside an escaped string), `<br>` and newline-entity joins, doubly escaped newlines, a block longer than the line cap, a lower-cased armour, and the RFC 4716 four-dash armour. (iss-2609020127210042, iss-2609251553082721)
- The capture redactor no longer masks the owner of the repository's own `owner/repo` slug; the owner alone, or as another repository's owner, is still masked. (iss-2608270645473170)
- The hostname rules stop flagging prose: a hyphenated slug ending in a device word (`migrating-to-the-nas`) is written as authored when it carries an English function word, and the `net_lan_hostname` and `net_device_hostname` suggestions name the exact accepted persona shape, `<persona>-<noun>` in lower case with no possessive. (iss-2609240646532741, iss-2609190338409222)
- The scanner's cost on match-dense lines is linear again: behind a network match only the secret patterns are sought, which removes duplicate IPv6 findings and cut a network-dense benchmark from 679 ms to 75 ms, and the per-match helpers that re-read the whole line are bounded, each bound failing toward keeping the finding. (iss-195, iss-2609251535277823)
- The launch payload scan bounds memory: a text file over the scan limit is reported as unscanned with its reason, `.gitignore` takes the full text rules, and findings past the cap are counted in `findings_omitted` while `hard_fails` still counts every one. (iss-2608291849371769)
- The release tag waits on the verify gate: `release.yml`'s tag job runs only after verify passes on the auto-release path, in abcd's workflows and both scaffold profiles, so a refused release leaves no tag and the version free; and auto-release refuses to rebuild a hand-pushed tag whose verify failed, naming the re-cut, while a failed publish still heals. (iss-2608231226347380, iss-2609251125599536)
- The release job's semantic receipt gate binds its receipts to the release being published: the derived content commit must carry the released tree's newest dated CHANGELOG version, so an earlier release's receipts no longer admit an unreviewed one; a tree with no CHANGELOG, no dated heading or a pre-release heading the reader cannot parse is refused; candidates are filtered by version before the nearest is taken; receipts directories must be named by a full 40- or 64-digit sha; and a scaffolded profile refuses a pushed tag that is not the tree's dated version. (iss-2609251755386183, iss-2609251939461459, iss-2609251939468296, iss-2609251939460232, iss-2609251939466588, iss-2609251945586202)
- `release.yml` and the scaffold template shape-check the tag as `vX.Y.Z` before the build, so a tag carrying a quote can no longer splice a shell command into `make build`. (iss-2608261041218890)
- The documentation citation gate is armed where a release is gated: the release workflow's verify job runs the docs lint with `--release-gate`, so a citation past its threshold blocks the release, while CI and `make docs-lint` keep the commit-time warning. (iss-2609091801085579)
...
- `abcd changelog` and `abcd launch ship` resolve the checkout root before the cut: from a subdirectory they read the same cut instead of an empty one with exit 0, and outside a checkout the refusal names the missing checkout instead of `exit status 128`. (iss-2609251713073532)
- The payload parity diff no longer reads a checkout missing the previous release's tag as a first launch: a tagless, shallow or stale clone whose CHANGELOG dates a newer release than its tags refuses, naming that release and the remedies (fetch the tags, `--fetch-baseline`, or `--baseline <tag>`). (iss-2609251902439938, iss-2609252001486609)
- `abcd launch --fetch-baseline` no longer unsets the proxy and CA environment for the whole process: the fetcher ignores those overrides for its own client only, and the report and a failed fetch's refusal name every override that was set. (iss-2609251902444497)
- The launch preview's retention plan refuses with its reason when the release-tag listing fails or the checkout is shallow, instead of planning from a partial tag set. (iss-194, iss-2609251238184553)
- The launch change-narration gate refuses genuine narration and passes present-state prose: `previously … now` is caught within a sentence and across a sentence end, a genuine `no longer`, `used to` or `renamed` hard-fails, while a participle `used to`, a relative or subordinate-clause `no longer` (`Retry until the error no longer appears.`) and a present-tense rename pass; the finding names the docs-lint escape. (iss-2609251827286563, iss-2609251940304726, iss-2609252045147575, iss-2609252045148890)
- The launch content gates no longer drop a document that opens with a `---` horizontal rule: a leading block is skipped as frontmatter only when it closes and reads as YAML. (iss-2609251827296447, iss-2609251940383450)
- A hooks config that is present but does not parse is now a finding of the launch hook-compliance row and a refusal of the install smoke, instead of passing as `ran, 0 concern(s)`. (iss-2609251827104081)
- The deep installability smoke accepts legal frontmatter (plain multi-line scalars, quoted and non-ASCII keys, and a block closed by the YAML document end `...`), so a page written in those shapes does not make `abcd launch ship` refuse. (iss-2609251902438821)
- The payload render refuses a dirty tree unless its caller opts out: `abcd launch archive` without `--verify` refuses an uncommitted change (exit 2) and its help says so, and the render-path documentation row reports `not_measured` instead of claiming no docs-lint config. (iss-2609251827294854, iss-2609251940377887)
- The plain `abcd launch --dry-run` render prints every gate row whose status is not `ran`, with its name, status and detail, not only the citation and receipt rows. (iss-2609251827290265)
- `abcd launch --dry-run` in a repository with no `.abcd/config/launch-payload.json` says the repository declares no launch payload and names the release path it does have, instead of a raw missing-include error. (iss-2608270559313719)
- `abcd spec close`, `abcd intent plan`, `abcd capture resolve` and `abcd capture wontfix` repoint every relative link that named the moved record's old path, closed specs included, and report each rewrite, so a successful move no longer leaves the tree failing `links_resolve`. (iss-2608311127491949, iss-2609091732329046, iss-2609250846525896)
- record-lint's `links_resolve` no longer reads a Markdown link inside an inline code span (`Get[T] (s, key)`), and a backslash-escaped backtick no longer hides a real broken link. (iss-2609250915305413, iss-2609251004336500)
- Fences and HTML comments are read by one CommonMark rule across the site, lint, ahoy, surface and memory readers: tilde fences, a longer fence quoting a shorter one, and commented headings no longer produce phantom sections, masked prose, missed findings or a mis-credited CLI reference; the site renderer refuses a fence it cannot render; a multi-line leading comment is no longer read as a record's title; and `abcd ahoy` places its marker block after the real H1 and refuses a file whose block would land inside an unclosed fence or comment. (iss-2609250955051598, iss-2609250955209513, iss-2609250955219864, iss-2609251044055902, iss-2609251045302499, iss-2609251514129841, iss-2609251517210637, iss-2609251518418878, iss-2609251522453961)
- The cold-reading exclusion floor no longer lets an excluded section travel in the bundle: its verifier reads headings independently of the redactor, and it refuses an excluded heading inside a fence both misread, behind a fence an HTML block swallows, behind a byte-order mark or a lone carriage return, or nested in a list item, blockquote or indent. (iss-2609250955207041, iss-2609251503459090, iss-2609251507443137, iss-2609251509209801)
- itd-199's three scope-condition identity markers are read as identities, so `abcd intent condition` reports each with its verdict disposition and can re-disposition them. (iss-2609250947113245)
- Writing into an intent's Audit Notes section leaves one blank line under the heading, not two. (iss-2609251053005833)
- Graveyard findings carry the binary's cap, shadow and truncation notices in a typed field, and every record-drawn graveyard string passes the shared prose cleaner, so crafted text cannot forge an omission notice. (iss-2608270926037088)
- The probe's truncation note names the bound that fired (the whole-walk cap, pruned depth, or an oversized directory) instead of always saying the rest of the tree was not walked. (iss-133)
- abcd's pre-push hook no longer runs the preflight while the push's connection is open: `make preflight` mints a receipt for HEAD only when the working tree matched HEAD at its start and end, and the hook refuses in milliseconds a push whose new commit has none, so a push can no longer report success while pushing nothing, and a partial commit fails locally rather than in CI. (iss-2608210738378295, iss-2608290810036869)
- abcd's own repository refuses a live session URL or a tool attribution footer in a commit or merge message before the commit exists: its committed `commit-msg` hook runs `abcd lint outbound` and fails closed when it cannot run. The managed-repository form of the hook is not part of this release. (iss-2609061438431625)
- abcd's own pre-commit name guard carries the scaffolded template's hardening (shell builtins pinned against inherited functions, a submodule path scanned, control bytes scrubbed from refusals), its pre-merge-commit hook carries the builtin pin, and the name guard prints one line naming every store it read instead of a notice per store per phase. (iss-2609250850380420, iss-2609181122202952)
- Help sentences match behaviour: the seven family parents (ideate, disembark, embark, history, guard, docs, implement) name their family's work and write discipline, `abcd implement load` names its unknown `--site` refusal, `abcd changelog` says it refuses outside a checkout, and the stale-surface refusal names each command whose sentence changed. (iss-2609251707308289, iss-2609251707302419, iss-2609251707308675, iss-2609251707317274)
- A mutually exclusive flag pair (`abcd ahoy --dry-run --identity`, `abcd update --check --yes`, `abcd site build --preview --version`) refuses as a usage error with exit 2, not exit 1. (iss-2609251734057081)
- The CLI reference no longer gives `abcd ahoy remote` and `abcd identity` a Usage line of their refusing bare form; it names the sub-verb form and the successor. (iss-2609251734069878)
- The load check's stray rule judges a process against its fair share of the machine as loaded rather than a fixed 0.9 of a core, so many long-running busy loops on an oversubscribed machine now warn. (iss-2609231947544298)
- Directory walks in probe and embark open each directory with no-follow and directory-only flags, so a directory swapped for a FIFO or device is refused instead of blocking the walk. (iss-337)
- Reading a home-scoped declaration (`~/.abcd/rules.json`, trusted-roots, path-entry and their kin) confirms the opened file is the one whose owner and permissions were checked, refusing a file swapped in between. (iss-2609251537550065)
...
- **A change that says it delivers an intent must ship it.** A commit trailer `Delivers: itd-N` is checked at merge beside `Resolves: iss-N` (RS005, run by `lint-issues` and in CI): the change is refused unless the intent enters `shipped/` in the same change, and the refusal names every intent left planned, every spec still open that names it, and the `abcd spec close <spc-N>` that clears it, in the same shape and exit code as the issue rule. What stops working: a change carrying the trailer with its intent still planned no longer merges, and a near-miss spelling such as `Delivers: spc-7` is refused rather than passed over; close the spec in the change or drop the trailer. A change that declares no delivery is refused nothing, so the intents already planned with open specs are untouched. (itd-2609111003026787, iss-2609091642508005)
- **The issue ledger's promote join is two-sided, under new names.** `abcd capture promote` writes `related_issues` on the intent and `related_intents` on the issue, and `abcd intent audit --issue-drift` reports a join that is broken on either side. What stops working: the back-links `promoted_from` (on an intent) and `promoted_to` (on an issue) are retired in the files and the JSON output; nothing reads them, and a record still carrying `promoted_to` is refused. A managed repository runs `abcd capture migrate --apply` once to rewrite them (without `--apply` it only reports). (itd-4, iss-2609231553391212)
- **Every feature release arrives with its own release page.** `launch ship` writes `RELEASE.md` at the repository root from the press releases of the user-facing intents that shipped in the cut: headlines told as prose, the rest listed by title, and persona quotes carried word for word and checked against their source. The outgoing page moves to `.abcd/development/releases/<version>.md`, and the page, the archive move and the dated changelog section land together or not at all. The composer's payload is refused whole on an unknown field, an oversize body, a malformed id, an id outside the set, a heading or fence, or a quote that is not verbatim, and the read-only preview lists the intents a cut's page is composed from. A cut of fixes alone leaves `RELEASE.md` as it is and says why. (itd-2609231013154443)
- **A session sees the records its peers hold before it mints or fixes one.** `abcd peers` (with `--json`) lists, read-only, the issues and intent drafts that sibling worktrees and local branches hold and this tree does not, uncommitted captures included, with home paths redacted. `capture resolve`, `abcd <record-id>` and `intent audit` name the peer's branch, path and folder instead of answering not found, and the status board carries one line when a peer holds something. A peer whose branch is merged or whose worktree is gone is skipped and counted, and a peer whose ledger holds one id in two status folders is marked unread with the remedy. (itd-2609091416295622)
- **A managed repository reports back to abcd.** `abcd report --template` prints an abcd-issued template to fill and writes nothing, and `abcd report [<file>]` validates a filled one and files it into an inbox in the user account, never into either repository's tree. abcd's session start says in one line how many reports wait and from how many repositories, and the status board carries the same row. `abcd inbox` lists them newest first naming the sender, `abcd inbox show <id>` renders one, and nothing is filed until `abcd inbox promote <id>` files it as a capture carrying the sender's root-commit key and a generic description in place of its name (a name that is a common word or three letters or fewer stays in the prose as a word). An abcd defect found while working in another repository has a home without routing it by hand. A report in a template version abcd does not know is listed as unreadable, naming the version. (itd-2609221656361680, iss-89)
- **Two sessions can share one autonomous run.** `abcd implement join` adds a second session without the first waiting on it; `mode` opens a window in one of three division modes (a claim per record, whole batches per session, or one session building while the other reviews and lands); `claim` takes a record exclusively under a lease that lapses, and holds the second session to one live claim at a time; and `check` refuses the second session a release step, a lane in a split-roles window, or a lane whose declared paths reach the reading corpus. `log` records the run's events, and `report` derives from the log, per mode, the lanes landed, wall clock, collisions, backoffs and agent minutes. The agent ceiling each session declares is recorded and reported, not enforced. (itd-2609221656373558)
- **Type the id, get your next move.** `abcd <id>` for an `iss-N`, `itd-N`, `spc-N` or `adr-N` finds the record in any status folder and renders, read-only, what it is, its key fields and links, and the next verb its lifecycle calls for; `--json` carries the same facts, and an id of that shape that no store holds exits non-zero with a diagnostic. (itd-121)
- **The brief's surface chapters carry a generated appendix.** Each surface chapter in the design record ends, under a marker, in a block listing the verb's flags and sub-verbs, derived from the same command tree as the CLI reference; a drift test fails naming the chapter when a flag or sub-verb changes and the block is not regenerated, and a check refuses a flag or sub-verb stated in the prose above the marker. `surface_coverage` names itself as the row-level presence check over the surfaces index. (itd-147)
- **abcd speaks one word for a specced block of work: `spec`.** The glossary resolves the concept to its `spec` term with `epic` only as a forbidden synonym, abcd-owned surfaces and prose say `spec`, and the `GL002` lint reports `epic` as a noun in any abcd-owned intent or doc. (itd-43)
- **Every abcd release publishes a pinned, version-stamped plugin archive.** `launch ship` renders `abcd-plugin-vX.Y.Z.zip` reproducibly from its tree and pins its release URL and sha256 in the plugin marketplace catalog; the release workflow re-renders it from the tagged commit with `launch archive --verify`, refuses a digest mismatch, and uploads it checksummed and attested with the binaries, so a plugin update installs exactly the cut release, verified against the pinned digest. A repository writes the pin only when its version-location contract declares `publishes_plugin_archive: true`. (iss-2609230733497536)
- `abcd intent plan <itd-N> --impact <additive|breaking|fix>` stamps the impact judgement onto an intent when the planning interview settles it, through the validator the close applies; a value that disagrees with one already recorded is refused before anything moves, and one that agrees is a no-op. (iss-2609170726457256)
- `abcd intent hold <itd-N> --reason "<text>"` records a hold as the frontmatter key `held`, and `abcd intent unhold <itd-N>` lifts it; `intent plan` refuses a held draft or planned record naming the reason and the lift, `abcd <itd-N>` reports the hold as the next move, and record-lint reports a held value in a shape no verb writes. (iss-2609200830076665)
- `abcd capture link <iss-N> --blocked-by <iss-M,...>` writes the `blocked_by` edge after capture, in any status folder, through the validator the capture flag runs, and `--unblock` removes it; the refusal for an absent target and the flag help on both verbs name where the field is documented. (iss-2609200951237670)
- A bundled `LOAD` rule domain carries three rules for any test or experiment that spawns background load in a managed repository: its children run as one owned process group killed together, clean is proven by what is actually running rather than by a pid file having been read, and a live development machine needs explicit consent and a cap below the core count. (iss-2609210828122412)
- The guard hook reads a host-supplied per-call working directory (`tool_input.workdir`), resolves it against the session directory, and checks the command against the registry of the repository it runs in as well as the session's, keeping the stricter verdict; a malformed working directory is refused with the blocking status. No adapter abcd bundles sends the field, so the path is reached only by a host that does. (iss-2609212142557657)

### Fixed

- **`abcd ahoy install` with default options writes no abcd-naming block into a repository's conventions files.** The docs target defaults to `skip`, so neither `CLAUDE.md` nor `AGENTS.md` gains the rule-loader block unless the project names `claude_md`, `agents_md` or both, and the adopted repository still classifies as managed on its registry entry; the dry run with an unset target no longer promises a block. The name-guard hooks and the `.gitignore` fence the install commits keep their abcd markers, the one sanctioned mention of abcd in an adopted repository. (iss-2609110944498549)
- **`abcd docs lint` says when nothing was checked.** With no rule armed, or roots holding no markdown document, it reports that nothing was checked (`checks: 0` in `--json`) and warns on stderr, still exiting 0, instead of printing a clean finding count. The docs-lint config an install scaffolds arms abcd's Writing-Guide `present_tense` and spelling families and the `links_resolve`, `harness_leak` and `stray_root_docs` rules, leaves banned names empty and the harness family to the repository, and seeds the em-dash-in-list-item token at the severity the adopter chooses, warning under an unattended install. (iss-2609150805167646)
- `abcd capture` refuses a `--found-at` that names a repo-relative path absent from the checkout, or one leaving it, before the ledger is touched; a conceptual location or an absent value is written as given. (iss-2609120511058115)
- `abcd <iss-N>` for a record whose file is in the ledger but skipped on read, such as one still carrying the retired `promoted_to`, names the file and the reason it was skipped, with the `abcd capture migrate --apply` remedy, instead of answering not found. (iss-2609240200426413)
- `abcd update` treats a PATH pin into a superseded plugin-cache version as abcd's own rather than foreign: it refuses naming the version the pin points at and the remedy `abcd ahoy install`, detection raises a resolvable `symlink.superseded` gap, and `ahoy install` rewrites the pin at the current release. A link beside a source checkout stays foreign. (iss-2609161805447092)
- `abcd intent "<text>"` seeds the draft's Press Release section with the quoted text and takes the title from the text's first sentence, cut at the slug cap; `--title` overrides it, and Why This Matters carries a prompt rather than the text repeated. (iss-2609170726360399)
- A record slug for a capture, an intent or a decision is capped between words at the sixty-character limit, so it never ends in a token its text did not contain: `ac-10` no longer lands as `ac-1`. (iss-2609211735419853)
- `abcd reading assemble` redraws a run id whose default output directory already exists, up to eight draws before a refusal naming the id, so two assemblies in the same second no longer refuse the second; a non-empty `--out` the operator names still refuses. (iss-2609211414208891)
- `origin: researcher-authored` names the route, text written directly rather than derived from another record, and asserts nothing about who invoked the command; the code comments, the capture command page and the records that read it as a claim about who ran the verb now agree. (iss-2609120505141653)
- Home-path redaction in rendered output and CLI errors redacts a root only where it starts a path, and redacts both the environment's and the symlink-resolved spelling of the home directory, so a home inside a longer path is left intact instead of rendered with a prefix stranded before the tilde. (iss-2609230641546141)
- The read-side memory lint (MR001) judges every page name, each page file's and each registry back-link's, orphans included, by the write side's own filename rule, so a store written before the write boundary refused such names reports a page name that embeds a secret token. (iss-2609090642035097)
- The memory lint judges a page back-link in the sources registry by the page-name rule alone, as the write side does, so an ordinary prose-shaped page name no longer draws an MR001 hostname-residue blocker. (iss-2609230851376499)
- The memory lint's `report.md` passes the store path and each finding's file, message and suggestion through the same sanitiser as the terminal render, so a `pii.json` pattern name carried by the degraded-scanner MR001 message reaches the file masked. (iss-2609020239068243)
- A memory page that opens with a byte-order mark parses with its frontmatter in every memory reader, as it does in record-lint. (iss-2608291814565781)
- The intent command page states that `abcd intent ready --grounds` records grounds on a draft or a planned intent alike, and refuses them on a shipped or superseded one. (iss-2609181020519987)
...
- **Every sub-agent a session spawns leaves the same redacted record its main thread does.** A sub-agent transcript is staged when the agent stops, at any nesting depth, and stored through the same fail-closed redaction a session transcript already had, with the spawning session, the agent, its type and its depth carried in explicit record fields rather than a hand-made composite identifier. (itd-2609090559376002, iss-2609081917287384)
- **`abcd history ingest --into <repo>` recovers transcripts already on disk into the repository that owns them**, redacted under that repository's own rules: the destination is an operand with no default, a transcript whose owner cannot be established is skipped and named rather than filed by guess, and `abcd history migrate` repairs the records filed under a composite session id. (itd-2609091718566731)
- **`abcd history reconstruct <session-id>` renders one whole session — the main thread and every sub-agent it spawned — as a single self-contained artefact**, beside a telemetry file reporting span, turns, tokens, tool calls, models and agent types per session and per agent, and stating its own gaps. (itd-2609091718595846)
...
- Staged raw transcripts stop living indefinitely: a small drain runs while a session is live, an overdue entry sorts to the front of every drain and is named in the notices, a session start surveys every repository in the store for a backlog, and a transcript that redaction deterministically refuses is quarantined with its reason instead of being re-read on every pass. (iss-2609090722466403)
- **A managed repository shows it is managed, and says whose answer the agent loop is waiting on.** `abcd statusline` leads the host's status line with one badge in three states — abcd is here and nobody is waiting, waiting on the facilitator, waiting on the product thinker — `abcd mode` sets the state, the install step offers and wires the line as its own consent category, and an unmanaged repository shows nothing. (itd-200, iss-168, iss-2608210934566225)
- **An intent owns one or more specs, and ships when its last spec closes.** `abcd spec close --remainder <slug>` mints the follow-on spec against the same intent, which stays planned. What stops working: closing a spec no longer ships its intent on its own, so a partially delivered intent reaches `shipped/` only at the close after which no open spec names it, and `--impact` is accepted at that close alone. (iss-2609100508566552)
- **A refusal in machine-readable mode answers on standard output**, in an envelope that announces itself with a top-level key no success envelope carries and states the exit status inside the document, so a consumer that merges the two streams can still tell a refusal from a result. What stops working: two verbs that render and then fail now put two documents on standard output where they put one on each stream, the refusal always last, so a consumer assuming exactly one document there must read to the end. (iss-2609100519128005)
- A refused flag value names the set it would have accepted: all three closed enumerations render their accepted values from the one schema when they refuse, and two of the three flags gained that set in their help as the third always had. (iss-2609100519128005)
- A commit message, pull-request title or pull-request body naming an `iss-N` must declare `Resolves:` or `Refs:` — RS004 refuses one that does not — and `abcd capture mentions` lists, read-only, the open issues named by default-branch history with no resolution behind them. (iss-2609100507421759)
- A record id written in a record's prose must name a record that exists: `record-lint` refuses an id that resolves to nothing, with a line-scoped illustrative or forward-looking marker as the escape and an id-keyed baseline carrying the corpus that predates the gate. (iss-2609100518527863)
- `abcd lint outbound` judges one piece of outbound text — a commit message, a pull-request body, a release note — against the outbound policy, refusing a live agent-session URL or a tool's attribution footer and masking the matched span in its report; it reports and refuses without rewriting the text, and is what CI runs over every commit message in a pull request's range and over the body. (iss-2609152131205316)

### Fixed

- **GHSA-4q78-ccfv-f374**: a plugin cache the environment names is promoted to the owned copy on PATH only when a home-scoped attestation — written by the bootstrap after authenticating the cache against the published release manifest — names that directory and the artefact hashes to the attested value; an absent or mismatching attestation is refused loudly and the install degrades to the pinned symlink. (iss-2609012039102770)
- The declaration naming which binary the hook shims may execute is honoured only when this session owns it and nobody else can write it: a symlinked, group- or world-writable, or foreign-owned record is ignored by the readers and by all five shims, which take their existing loud refusal instead of executing a binary another local account chose. (iss-2609091927085132)
- A reverse-DNS identifier or module path whose first component happens to equal the account name survives capture intact: a bare-username match is suppressed only where it is a whole component of a dotted run of at least three, while a bare mention, a two-part filename and an email local part still redact. (iss-2609100505142469)
- Identity redaction rewrites exactly the byte spans the detector flagged, so a line carrying both a genuine mention and a lookalike keeps the lookalike byte for byte instead of having it masked as well. (iss-2609120446083912)
- A transcript ingest refuses a destination whose repository root and store key name two different repositories, so one repository's redaction configuration can never govern another's corpus; a root whose own root commit does not resolve refuses rather than passes. (iss-2609091911060345)
- Nothing quoted from a session can begin a line of the reconstruction artefact: every scalar written outside a fence goes through one line cleaner, is placed in a code span whose delimiter outruns any backtick inside it, and is pipe-escaped in the timeline table, so the document's structure is the document's own. (iss-2609091913570877)
- The refusals that protect a write from a degraded secret scanner are armed by tests rather than merely present: transcript capture, the composite-id repair and the memory store's write side each prove they refuse instead of storing under weakened redaction, and the sweep armed every other such site bar the two that are report-only by design. (iss-2609091915350221)
- A sub-agent transcript is placed by its session before its own recorded directory, so an agent whose isolated worktree was removed when it stopped is filed under the repository that owns it instead of being orphaned. (iss-2609090723023943)
- Transcript telemetry counts one usage per response rather than per transcript line, so a response written as several content blocks is counted once instead of inflating a session's reported cost by a factor that varies from session to session. (iss-2609090723027424)
- The privacy-hygiene lint stops firing on the two shapes the conventions themselves mandate — a persona home path from the committed registry, and anything beneath a shared system root — while a non-roster username, a persona used as a prefix or suffix, a lookalike directory, a traversal escape and the committing user's own name all still fail. (iss-2609100505145554)
- Deleting a blocking finding no longer clears the release gate: a record the anchor held open and that is absent from the whole ledger at HEAD refuses the cut by name and grade, while a resolution, a wontfix, a re-slug and a standing deferral each go on clearing it. (iss-2609091143455568)
- An id claimed by more than one record file is refused by every read of the ledger, naming both paths, so a merge that leaves a record in two status folders can no longer read as open and resolved at the same time. (iss-2609100507430423)
- A link on PATH that resolves to nothing is cleared by the install step instead of walling it for ever, while a link that resolves to a file is somebody's working install and is still refused untouched. (iss-2609100506256636)
- A repository whose artefact is not a plugin can use the release flow: an absent plugin manifest reads as absence rather than an unreadable payload, so the derived changelog, the payload scan and the release gates run, while a manifest that is present and unreadable or malformed still refuses. (iss-2609100506255436)
- An intent-fidelity request carries host-computed rubric and prompt hashes and the ingest verifies them rather than shape-checking them, so an audit note can no longer record provenance the host never issued; a hash the host did not issue is refused outright and the owed review stays open. (iss-2609100505140261)
- `abcd spec close --impact` stamps the product impact on an intent that declares none, validates one already recorded at the same bar and refuses a flag that disagrees with the record, so the missing field is answered in the same command instead of sending the operator back to edit frontmatter by hand. (iss-2609100518522943)
- A mistyped `intent` sub-verb is refused with the registered sub-verbs named and nothing created, instead of being filed as a draft intent: a whitespace-free token followed by a record id is judged a subcommand call, while genuine prose of two or more words still files. (iss-2609100508562701)
- `abcd decide` mints a decision record through the same collision-proof allocator every other record family uses, so a repository whose own records tell an author to run it can be followed literally instead of hand-minting an ADR by copying a neighbouring file's shape. (iss-2609100507432904)
- The ledger's human gates report rather than refuse where the language they ask for is not yet settled: `intent ready` keeps its seven rows and gates on four, `capture promote` and `capture resolve` record grounds only when given, and a lapse capture with no instant records none. (iss-2609091009111294)
- The README and the roles page name the same three roles and state the intent-fidelity judgement as recorded rather than enforced, so the front door stops claiming as achieved what is still the design intention. (iss-2608290833558547)
...
- **A hook shim runs an `abcd` from PATH only when you own it.** All four shims that carry the PATH rung (UserPromptSubmit, PreToolUse, PreCompact and SessionEnd) accept a PATH-resolved binary only when `~/.abcd/path-entry` records that exact path as this machine's installed abcd, after the existing absolute, outside-the-working-tree and not-world-writable checks; a foreign binary gets the shim's loud one-line refusal naming it and the reason, and for PreToolUse that is UNGUARDED and exit 1 rather than the exit 0 a harness reads as an approval. What stops working: a PATH copy installed by an install one-liner from an earlier release carries no such record and is refused until the one-liner is re-run, which writes `path=` and `binary_sha256=` for you. The record is home-scoped and same-uid writable, so this is ownership against a hijacked PATH, not verification against a compromised account. (iss-2609012039107700)
- **A configuration root owned by another user is refused.** Where git will not answer for a tree, the rules resolver refuses a repository-shaped marker root whose owner is not the caller, fails closed on the working directory with the bundled rules and the bundled hazard registry, and names the remedy, so a repository planted in a shared ancestor cannot supply `.abcd/rules.json`, the loader kill switch or `.abcd/guard.json` to a session beneath it. What stops working: a legitimate foreign-uid checkout, a CI runner or a container bind mount is refused until its root is declared in `~/.abcd/trusted-roots`, which is read only when it is a regular file this uid owns that no one else can write, so the tree under judgement can never assert its own trust. (iss-2609020259564193)
- **`abcd spec close` states the shipping judgement.** It takes `--impact`, stamps a supplied judgement onto an intent that carries none, validates one the record already carries at the same bar the seed applies, and refuses a flag that disagrees with a recorded judgement. What stops working: closing a spec whose intent declares no impact, with no `--impact` supplied, refuses instead of shipping a record that the record gates then reject, so a plan-and-ship sequence driven entirely by abcd's own verbs either records the judgement or says it is missing. (iss-126)
- **A cold reading is commissioned with two things: where it stands and what it reads against.** `abcd reading assemble --position <position> --target <commit>` is the whole invocation, what the reading is handed comes from the committed preset entry for that position, and the manifest records the entry applied and its hash, so a run is reproducible from the commit it names. What stops working: the `--scope` operand and its override stamp are withdrawn, and a third operand of any kind is refused by name, so widening or narrowing what a position reads is a reviewed commit to the preset file rather than a flag typed at the prompt. (itd-2609021003095168)
- **The comparative reading can be commissioned.** `abcd reading assemble --position comparative --target <commit>` derives from the record the one committed widening run at that target whose items carry no disposition and no admission, and hands the reading that run's returned configurations, projected to the two fields the widening body carries, together with the six declared selection criteria and nothing else from the readings store; if no run qualifies, or more than one does, or any item already carries a fate, or the run returned fewer than two configurations, the assembly refuses and says which. The derivation reads "at the target" as reaching an ancestor when every path changed between the two lies inside the durable readings store and the issue ledger's own record families, so committing an ingest's records, the act the design sequences between an ingest and the next reading, keeps the run eligible, and the comparative manifest carries `candidate_run_target` beside `target_commit`. A reading that returns no items commits at every position as a run with an empty item set, while a run whose every item was refused stays a list-level refusal, because those are different facts. (itd-2609020625407419, iss-2609021833302981, iss-2609021153269181)
- **The record's own vocabulary is committed.** Nine core glossary entries fix one spelling per sense for phase, plan, roadmap, record, surface, ledger, construal, reading-position and loop, each with a repo-relative link to the chapter it lives in, and the roadmap phases README and the intent surface page each point at the entry for the word they use; a ledger bounded context commits nine more terms as they presently stand, so the vocabulary a widening reading is handed contains the words the construal is written in. (iss-2609012245352480, iss-2609012259586524)
- **The site export carries the glossary.** `/record/glossary/` renders one page per entry in the explorer's navigation, and the first use of each entry on a rendered record or entry page links to it, aliases included, case-insensitive and whole-word, with dedup by entry rather than by spelling so a term's aliases together earn one link. A use inside a code span, a heading, an existing link, or on the term's own page is left as the record wrote it, and a repository keeping no glossary gets no pages, no navigation entry and no links. (iss-2609020922150364)
- **`abcd decide "<title>"` mints a decision record.** It allocates `adr-<yymmddHHMMSS><rrrr>` through the same record-id seam issues, intents and specs use and writes the ADR skeleton to `decisions/adrs/`, so two branches deciding on the same day cannot allocate one ordinal for two decisions. One derivation serves every reader of an ADR id or filename, so the hand-numbered ordinals and the minted stamps resolve, lint, sort and dispatch side by side, with ordinals ahead of stamps in the index order. (iss-2609012206053814)
- **The cold-reading exclusion floor names the local ledger tier.** `.abcd/.work.local` is a declared floor entry citing brief invariant 14, so every manifest asserts that framing traces and declined construals were refused and the assertion is enforced rather than left to a reader's inference. The core assembler version reaches 1.8.0 in this release, the floor entry taking it from 1.2.0 to 1.3.0 as the first of six steps, an added floor entry being an added promise rather than a rewording, and the charter's rendered table carries the row. (iss-2608311238236490)

### Fixed

- **Every record-writing verb addresses the checkout's own store.** capture, decide, spec, memory, intent and ideate each resolve the checkout root through one promoted primitive before they build a request: run from a subdirectory they address the checkout's ledger, decision store, spec store, intent store, memory substrate or verdict record; run outside a repository, or in a repository-shaped tree git will not answer for, they exit 2 with nothing read or written; and a store already sitting below the checkout root is named on stderr and left untouched. The cost the resolution removes is a record filed where no gate, no release cut and no reader looks, written with every appearance of success: a read from `internal/core` answering with an empty ledger against 365 open records, and a write minting a second store under that directory behind a repo-relative path that reads exactly like the checkout's own. (iss-2609090951291524, iss-2609091707224329, iss-2609091729516940)
- **A wired SessionEnd hook stores a transcript on a machine where `ahoy install` never ran.** The transcript store is user-level, self-creating and keyed on the repository's root-commit SHA at `~/.abcd/transcripts/<root-sha>/`, with `records/` and `staging/` beneath it, so capture carries no install precondition and a session ends with a stored transcript rather than a stderr line and exit 0. A per-repo location is an opt-in pull declared in `~/.abcd/local-transcript-roots`, home-scoped, uid-owned and not other-writable, landing in the gitignored `.abcd/.work.local/transcripts/`; a corpus at `~/.abcd/history/<root-sha>/transcripts/` is moved into the store on first resolve, reported on stderr, and tombstoned at the old path. (iss-95)
- **Every install route that leaves an abcd on PATH records it.** All three install one-liners write `path=` and `binary_sha256=` into `~/.abcd/path-entry`, and `ahoy install` stamps the record for its pinned symlink and its `--dev` shim as well as for the owned copy, under the spelling `command -v abcd` prints and the hook shims compare, so the bootstrap refreshes such a copy on the next release and the status board owns it instead of classifying it foreign. An owned entry the record does not name raises a required, resolvable `symlink.unrecorded` gap, so an install that reports itself healthy while every hook prints its degraded-route refusal can be healed by re-running the verb; uninstall drops the record with the entry it names and only that one, so nothing that later occupies the vacated path inherits the ownership claim. (iss-2609012111159045, iss-2609091126475539)
- **`abcd update` replaces the running binary when its release has been deleted from the forge.** Ownership is proven first from the running executable's own identity (`os.Executable` with symlinks resolved, then `os.SameFile`: the bytes that loaded the code asking the question cannot be a foreign binary) and then from the `~/.abcd/path-entry` install record, neither of which a release deletion can revoke; the published-manifest walk dates the file rather than proving it, and a file it cannot date is reported as an unpublished build with its digest. The refusal that remains reinstalls over the file and names the one-liner and `ahoy install`, rather than directing the reader to delete the only tool that could reinstall. Pre-1.0.0 binaries are out of scope: the mechanism is fixed, the historical population is not rescued. (iss-2609012000222546)
- **`ahoy install` run from a terminal reaches the verified cache.** The bootstrap stamps the data directory it provisioned the plugin root from into `<plugin-root>/.data-dir`, and the resolver falls back to that stamp, validated as an absolute existing directory, when `CLAUDE_PLUGIN_DATA` is unset, so the install the guide tells a reader to run from their own shell lands the owned copy rather than the pinned symlink that dangles after the next plugin update. The artefact is still re-hashed against its recorded `binary_sha256` before any copy, and the degradation note names every source tried. (iss-2609012111168716)
- **The bootstrap says what it declined to do.** The migration seed prints a notice when it refuses a root binary whose hash does not match its recorded provenance, and the PATH-copy refresh names the copy it left untouched, in tilde form, with the reason, so a copy that has quietly stopped being refreshed is a line on stderr rather than something a reader discovers when `abcd update` or the status board calls it foreign. (iss-2609012111160075)
- **A mistyped sub-verb refuses instead of minting a record.** `abcd intent` and `abcd capture` decide on the shape they already judge: a whitespace-free token followed by a record id is a subcommand call whatever its edit distance, so it exits 2 and lists the registered sub-verbs rather than filing the typo as the title of a new draft intent or a captured issue in the durable tier. Genuine prose of two or more words still files, and the near-miss did-you-mean, the retired-spelling successor and the lone-token message are unchanged. (iss-2609091647589392)
- **A multi-line refusal renders as the lines it was written as.** The release surface routes a multi-line reason through the block sanitiser, which neutralises a control sequence while leaving the line structure intact, so the surface guard's list of what narrowed since the anchor tag, the stale-intent refusal and the findings gate's enumeration of the records blocking a cut each read one record per line at the terminal instead of one run-on paragraph with question marks where the breaks belong. A single-line reason keeps the single-line sanitiser, and an escape sequence is still neutralised. (iss-2609091046297503)
- **`abcd ideate record` refuses rather than committing an unscanned verdict.** Every free-text field of the verdict, the idea, each claim and its primary source, each grill note, each kill attempt and each rejected alternative with its reason, is redacted on the way into the validated document, ahead of the terminal neutraliser and well ahead of the renderer, with the deterministic literal-`$HOME` backstop the history and memory stores apply on the same boundary; the fully rendered record and the composed decision-log pointer line are then re-scanned together and any blocking residual refuses the write. A scanner that is unavailable or degraded fails the run closed, which is the posture of the committed tier ideate writes into, and because the record write is a one-shot exclusive create, a refusal leaves no partial document behind. The result carries a redaction count, so a composer learns it has just pasted a credential. (iss-2608291817368607, iss-2609020127281995)
- **Agent-supplied prose is redacted before it reaches a committed record.** The intent-audit ingest routes per-criterion rationales, gap-audit claims, scope-condition rationales and narrowings, evidence refs and quotes, and the dead-letter reason through the privacy redactor before the terminal neutraliser, with the scanner built once per ingest and failing closed before any block is composed. A field is excused only where a validator constrains it: `policy.rubric_hash`, `policy.prompt_hash` and a present attestation digest must read `sha256:` followed by 64 lowercase hex or the payload is quarantined, while `verifier.id`, `verifier.version` and an attestation's kind and ref take the same prose redaction the rationales do. The reading ingest does the same for every payload-derived string bound for `run.json` or `refusal.json`, including a refused item's criterion and candidate id, the unknown and reserved field names, the out-of-vocabulary value and the recorded instrument identity. (iss-2608300924205748, iss-2609022002241168)
- **A memory page filename is judged at the write boundary and refused, never rewritten.** The store redactor judges the joined page name, the three components the name parses into, and every suffix beginning immediately after an underscore, at a hard-fail-secrets bar, so a credential whose own prefix straddles a separator, `topic_ghp_` followed by its body or a Stripe live key split across domain and slug, is caught where a component split falls exactly at the missing word boundary. A refused page reaches neither the file, `index.md`, `log.md` nor the registry back-link, and an ordinary page whose domain is a short real word such as `api`, `git` or `ghp` itself still writes. (iss-2609020321100138, iss-2609090953184203)
- **A truncated or narrowly rewrapped private key is redacted.** A visible body of sixteen to thirty-nine characters opens a PEM block on two grounds that both have to hold and that width says nothing about: the header was pasted, its marker being the whole content of its line rather than named inside a sentence that runs on past it, and the run carries a byte of the base64 alphabet outside the letters, a digit, a plus, a slash or the pad. That covers the key a mail client, a terminal rewrap or a log rotation leaves as a header plus one short line, without readmitting a mentioned header opening a block on any sixteen-character word. The cost is stated in both directions: a real body of that width is still missed where its header carries prose after the marker, or where its run happens to be letters only. The armour, characteristic-width and consecutive-pair routes are unchanged, and no entropy bar is read. (iss-2609090932441377)
- **A secret inside an archive or a compressed image chunk is a finding.** The payload scan decodes zip, gzip, bzip2, zlib, tar and PNG's compressed chunks (IDAT, zTXt, a compressed iTXt, iCCP) through the standard library alone, keys the format on magic bytes rather than on the filename so a zip renamed `.png` is labelled and decoded as a zip, and scans what comes out with the same byte rules. Each walk proves the bytes were read rather than assuming it, and decoding draws on one budget per payload file, 4 MiB of inflated output, four nesting levels and 1024 entries, so a bomb is refused rather than exhausting the host. A format with no standard-library decoder, including xz, 7z, zstd and any unrecognised magic, stays named as content-unverified rather than reported clean, and the report carries the decoded tier, the unverified tier, the reason per unverified path and the magic-keyed format separately. (iss-2608291832160371)
- **An install answer of `yes` never switches the deep secret scan off.** The collect-missing `scan_deep` slot re-checks the prompter's answer against its declared choice set and returns the partial-install path on anything else, naming the unfilled slot, exactly as the `docs_target` and `oracle_backend` slots beside it do, so an unparseable answer cannot resolve to a weaker scan than the one the operator asked for. An explicit `false` still disables the deep scan deliberately, the choice set and its default are declared once, and a refused prompt takes the documented default. (iss-2609090642031172)
- **A record gate reads the value a YAML reader reads.** The one same-line frontmatter scanner strips a trailing comment, starting at a hash preceded by whitespace and outside single or double quotes, so a hash inside a quoted value or with no whitespace before it stays part of the value, and every gate and every store inherits the strip rather than each predicate anchoring on the last byte. Emptiness is decided by the class of node a value spells, null, empty flow collection or empty string, after reading off its tag, anchor and alias properties, so `!!null null` and the fully spelled null tag answer as `!!null` does, and no eleventh literal has to be enumerated. No YAML dependency was added. (iss-2608301744268001, iss-2608301808198621)
- **A committed lint config cannot walk the record gate outside the repository.** Every configured repo-relative path is checked for containment at config load, through the canonical relative-path predicate rather than a second one, refusing anything absolute, unclean, backslashed, or climbing out with `..`: the top-level roots list, each rule's sixteen path fields, every `record_stores` key the config writes, and each index-drift document and directory pair. A value that escapes never reaches a path join, so a gate cannot be pointed at frontmatter outside the checkout and made to echo it. (iss-2608301308367566)
- **An inlined drawing cannot style the published page.** The site's SVG attribute gate holds an inlined drawing to a closed allowlist built from the committed drawings plus the standard presentation and geometry set, refusing `style`, `class`, foreign namespaces and anything unweighed by name, alongside the existing scheme checks on `href`, `xlink:href` and `src`. Inline CSS on the root element cannot beat the page's own rules, and a child cannot position itself to the viewport. (iss-2609081940398387)
- **Every site manifest path field is held to a closed set.** `home.hero.page` and `home.chapters[].page` are closed to `docs/` and `site-src/`; the quote-type fields, `identity.file`, `ui_strings`, the deferred feature quote paths, the contributors policy file and the unresolved-reference baseline, each take a closed set declared for that field; and every manifest path field refuses the git directory through the shared gate. So a manifest edit cannot compose `.git/config`, a dotenv file, or a path under the gitignored local tier into the published site, which matters most for the contributors policy file, whose non-first-bullet parts render the whole matched section verbatim. `.abcd/site.json` is code-owned, and the committed manifest still loads. (iss-2609081940475662, iss-2609090951279243)
- **A human author keeps their commits on the contributors page.** The bots row demotes on a structural machine signal, a `[bot]` name suffix, or a vendor-name match together with a machine mailbox, so a conformant `Assisted-by:` trailer whose vendor token happens to equal a single-token git author name cannot move that person's whole shortlog into tools, while a vendor at a noreply address stays where it belongs. The model chart tallies only trailer-shaped values, clipped to the attribution gate's own grammar, so a free-form second trailer line is not published as a bar label. (iss-2609081940550352)
- **The homepage features an intent whose audit actually passed.** The acceptance rollup is read from the first `Audit Notes` section alone, through the package's own fence-aware section walker, a negative count is refused outright, and a document carrying a second section of that title is treated as malformed, so neither a fenced rollup line nor a duplicated section can lift a concerns-only audit onto the feature block. The real tree's met set is unchanged at 26 of 41 shipped intents. (iss-2609081941073604, iss-2609090951277880)
- **The homepage feature block credits the release that shipped the record.** The changelog walk reads every handle on a line out whole through the package's own handle pattern and compares it entire, so `itd-1990` cannot be read as a credit for `itd-199`; it disqualifies a handle glued to a hyphen on either side, so a compound such as `fix/itd-199-cleanup` credits nothing; and it tracks fences ahead of the dated-heading test, so a handle inside a fenced block is not a credit and a fenced dated heading moves no version cursor. Newest-section-first is unchanged, and the committed changelog's own credits are corrected, `itd-1` taking 0.2.0, the release that names it. (iss-2609081941074556, iss-2609090951280114, iss-2609090951287232)
- **One declaration governs the Go toolchain and the format gate.** Every `setup-go` step in the four workflows, and all three in the scaffold template, resolve the toolchain from `go.mod`, so bumping the go directive moves every lane and the format gate together with no workflow edit and no scaffold-time snapshot to go stale. The release verify job invokes the same `make fmt-check` definition CI invokes, and the scaffold template's bare branch runs the gofmt under the GOROOT the adopter's own declared toolchain resolves to, so a managed repo receives a pinned format gate as its starting position. The roster is derived from the tree rather than hand-listed: a sweep over every workflow and every scaffold template refuses a literal version pin and a gofmt that PATH resolves. (iss-2609090951291799, iss-2609091128354325)
- **One create-then-prove directory primitive, with the weakest caller hardened onto the strongest guarantee.** `fsutil.EnsureRealDir` is the create-then-prove step and `EnsureRealDirAll` is that step walked over a chain, with the create mode as a parameter, so private stores keep 0o700 and the record store keeps 0o755 and no directory on disk changes. The chain form proves every level as it creates it, closing the symlinked-ancestor hole the intent store's own variant recorded in its doc comment, and the name joins the canonical-primitive detector's list so a fourth copy cannot land quietly. (iss-2609091128479544)
- **A cold reading's manifest says exactly what was examined.** The include table and the exclusion floor describe one set, item by item: every row carries a declaration of whether the floor parses what the row admits, a markdown document the floor cannot resolve is refused at admission and named rather than travelling unscanned, and a source or test file, which the floor does not parse, is admitted only when a committed preset entry opts it in, travels whole, and is marked `unscanned` in the manifest. The key and heading exclusions are asserted per item and only for the items the floor parsed, so a scan that ran and a scan that never ran produce different artefacts, and a record-shaped document inside a Go fixture arrives disclosed rather than silently. The brief's surfaces, internals, delivery and meta chapters join the object as brief current text, the evidence chapter staying excluded with its ground stated in the table, and the shipped intents leave the widening position, which neither design document lists in its object, the floor asserting that withdrawal. (itd-194, iss-2608301450065320, iss-2609021153264023, iss-2609012259587904)
- **Five ways an excluded key or heading could travel into a reading bundle are refused by shape rather than by spelling.** A delimited block that does not open at line 0 but opens one to a reader of the bundle, because only blank lines, whitespace, a byte-order mark or an HTML comment precede it, is refused and named with its displacement, while a delimiter after real prose stays a thematic break and opens nothing. An explicit key in a flow mapping is refused whatever the key is called, beside the nested block-sequence mapping. The fence mask starts after the frontmatter block closes, so nothing inside the block can switch the excluded-key scan off, and a fence delimiter inside the block is itself refused. An attribute value that opens on the line after its equals sign is refused rather than guessed at. A CRLF blank line bounds a raw heading element as an LF one does, and an opener that reaches the end of the document with neither a hard nor a soft bound is refused as a heading element that is never closed. (iss-2608301237456350, iss-2608301251398360, iss-2608301350533102, iss-2608301350534164, iss-2608301421380392)
- **The assembler runs over this repository's own corpus.** The markup-shape refusal reads the unfenced body, with fenced lines blanked rather than dropped so a match offset still counts the document's own newlines, so an admitted document showing an HTML attribute in a code block assembles while the same shape outside a fence still refuses. The 21 glossary records open with their frontmatter delimiter at line 0, the attribution comment sitting verbatim on the line immediately after the block's closing delimiter, so `reading assemble` refuses at no assembling position over the committed tree. (iss-2609021954509554, iss-2609021457186209)
- **A committed preset entry states what a position is handed and what it costs.** Each entry names the object set the run is about, the kinds admitted within it, and the estimated-token window it was calibrated for, beside the figure measured and the commit measured on, and changing any part of it is a commit. Each position is handed its own entry, so the three assembling positions' default item sets are pinned by digest and asserted distinct, and two assemblies of one entry produce byte-identical bundles: what a position reads moves by a commit to the preset file and by nothing else. The drafts and planned rows narrow by the entry's record list exactly as the shipped row does, with the readings companion's admissibility surviving as an entailment-only `admit_drafts_and_planned` switch that defaults off and is refused at load anywhere else, so the entailment position is handed the ten Iteration 2 intents beside the fifteen workstream intents rather than all 147 on the tree. An eval dry-run-assembles every assembling position on a clean clone of HEAD and fails past the declaration, naming the position, the measured figure and the declaration, and the size report states any total over the two-hundred-thousand-token target. (itd-2609020625400445, iss-2608311501186646, iss-2608311501240566, iss-2609021748300398)
- **The entailment size report states the mechanism proportion.** Counted per file from the candidates by path and field, it says how many projected intents carried a `## Mechanism` section, how many carried the `None stated.` nullity, and how many carried neither, which is the yield bound the readings companion asks to be reported beside the findings rather than discovered afterwards. No other position's report carries the statement, and no shipped intent is backfilled. (iss-2609012259585189)
- **An intent a reading occasioned says so, from both ends.** Promoting an accepted reading item into an intent draft writes the draft's `origin` as `contributed-by-reading` naming the run and the item, and stamps the item's `promoted_to` pointing forward, so the genealogy is readable from the record rather than from the commit history, and the provenance lint that resolves that pair has a writer producing it. Promoting an issue keeps saying `extracted-from-record`, because an issue is something a person noticed and a reading item is something an instrument returned. (itd-2609020625400169)
- **The four cold-reading definitions state the ordering rule the companion states.** The blindness core's fourth condition opens "Items are returned in the order they arise in the object", byte-identical across the four definitions with the rest of the condition unchanged, and the four `prompt_version` values move together with one entry naming them. (iss-2609021153261145)
- **The comparative dirty gate covers the records the derivation reads.** Any dirty path under the ledger's dispositions or admissions family counts as included at the comparative position, both roots derived from the ledger's own path constants rather than named literally, so a committed disposition deleted in the working tree alone, or an uncommitted admission that would disqualify one of two otherwise-ambiguous runs, refuses the assembly and names the record. A manifest therefore never promises a re-run that would derive a different candidate set, and a clean tree carrying committed fate records still derives. (iss-2609021950351082)
- **A committed reading run cannot turn the regime guard red.** The operator-surface walk skips the readings record family, derived from the schema's own directory constant rather than a literal, so `run.json` and `refusal.json`, which carry the regime the design requires the position's definition to stamp, are read as records a verb writes rather than as configuration an operator could set, while a regime key planted under `.abcd/config/` is still reported. (iss-2609022252265901)
- **The reading verb's help and its plugin page describe the code.** The bare verb's `definitions` field is what the definition locator resolved, and a definition silent about its position or its regime, or stating another position's, refuses the whole verb with exit 2 rather than being listed. The ingest verb's help says that a refusal after the run's identity is proven writes its refusal record and rolls back that run's own never-committed reading records, qualifies the durability claim to nothing durable because the stage root and its lock are created first, splits the orphan sweep into its two outcomes, and points the reserved-name rule at the table the gate reads. Each sentence is held to observable behaviour by a test, and the generated CLI reference and the brief chapter carry the same corrections. (iss-2608311039586922, iss-2609020843067458)
- **A refusal names only the stages that still stand.** The ingest result's `pending_stages` subtracts the stage the run's own rollback clears, so the disclosure a refusal renders names only the stages still standing after the invocation, in exactly the case that disclosure exists to describe. (iss-2609020848468450)
- **A reading refusal reads as one prefix.** Every `reading` sub-verb composes its refusal from the verb name plus the core message with the core's own `reading: ` tag trimmed, through one helper, so the terminal shows a single tag rather than `abcd: reading: reading: ...`, in text and in `--json` alike, and the core stays the canonical owner of the tag its refusal records and tests read. (iss-2608311145286014)
