...
// newReleaseFetcher is the seam through which `update --check` reaches the
// network. It is a package var so a test can inject a recording stub and prove
// two things: the check path fetches exactly once, and NO other path
// (`--version`, ahoy, session-start, install) ever constructs it — the
// zero-network invariant of adr-38 tier 1.
...
// checkSource names the network source `update --check` consulted, so the
...
func runVersion(cmd *cobra.Command, asJSON, check bool) error {
	cwd, err := os.Getwd()
	if err != nil {
		return err
	}
