...
func TestSessionRepoRoundTrip(t *testing.T) {
	_, _ = setupStore(t)
	if err := NoteSessionRepo(testRootSHA, "sess-note"); err != nil {
...
func TestSessionRepoRefusesAnAmbiguousSession(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(home, ".abcd", "history", other, "transcripts"), 0o700); err != nil {
		t.Fatal(err)
	}
	for _, sha := range []string{testRootSHA, other} {
		if err := NoteSessionRepo(sha, "sess-both"); err != nil {
...
func TestSessionRepoRefusesADirectoryReference(t *testing.T) {
	...
	for _, id := range []string{".", ".."} {
		if err := NoteSessionRepo(testRootSHA, id); err == nil {
...
func TestSessionNotesArePruned(t *testing.T) {
	_, home := setupStore(t)
	if err := NoteSessionRepo(testRootSHA, "sess-old"); err != nil {
		t.Fatal(err)
	}
	dir := filepath.Join(home, ".abcd", "history", testRootSHA, "sessions")
	old := time.Now().Add(-2 * sessionNoteTTL)
	if err := os.Chtimes(filepath.Join(dir, "sess-old"), old, old); err != nil {
		t.Fatal(err)
	}
	if err := NoteSessionRepo(testRootSHA, "sess-new"); err != nil {
		t.Fatal(err)
	}
...
func TestSubagentGapMarker(t *testing.T) {
	_, _ = setupStore(t)
	if _, ok, err := SubagentGap(testRootSHA); err != nil || ok {
		t.Fatalf("a fresh store reports a gap: ok=%v err=%v", ok, err)
	}
	if err := NoteSubagentGap(testRootSHA, "SubagentStop"); err != nil {
		t.Fatalf("NoteSubagentGap: %v", err)
	}
	note, ok, err := SubagentGap(testRootSHA)
	if err != nil || !ok {
	...
	again, _, err := SubagentGap(testRootSHA)
	if err != nil {
		t.Fatal(err)
	}
	if again.Count != 2 {
