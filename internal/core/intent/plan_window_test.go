...
// The same sibling in Plan's in-place link of a planned record with no spec:
// the kind it keeps is the one the record carries when it is read under the
// lock, not the one the corpus held before it.
func TestPlanInPlaceKeepsAKindChangedInTheWindow(t *testing.T) {
	root := t.TempDir()
