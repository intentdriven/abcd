...
- 2026-09-25 — The launch change-narration gate's hard tier refuses where the words cannot tell narration from present state, rather than passing (fix round 2 of lane launchgates, on iss-2609251940304726; the itd-65 acceptance criteria stand as written: "previously X, now Y" and a genuine "no longer" hard-fail). "no longer" is refused whatever its subject, except as a comparative ("no longer than") or in a relative clause over "is"/"are" ("files that are no longer present"); "renamed … to" is refused whenever a "to" follows before the next punctuation mark, except the present purpose form "is/are renamed to match"; "previously … now" is refused when "previously" opens its clause or a change verb stands within three words (counting words only, not punctuation or determiners); "used to" is refused as the past habit, including after a time adverbial ("until v0.6 the dry-run used to skip the tags"). The residual trade, refused although it states present state: a present passive rename other than "to match" ("the staged file is renamed to its final name"); a reduced or copula-less relative "no longer" ("files no longer present are skipped", "a seam that no longer exists"); a clause-opening "previously" beside an unrelated "now". Each takes the docs-lint escape on its line, which every finding names. Survey at the change: 0 findings in the 15 shipped doc bodies; over every Markdown file in the tree the finding locations go from 74 to 405 (333 added, all in records, research notes, agent instruction files and the changelog, outside the gate's scope; 2 dropped, each a "renamed" with no "to" after it). Rejected: a fixed list of change-subject nouns (round 1), which passed "the registry can no longer be edited" and "it was renamed to abcd lint", so a genuine narration reached a release with neither tier catching "used to" or "renamed to" (implementer of lane fix2-launchgates).
- 2026-09-25 — The launch change-narration gate's hard tier closes its heuristic rounds with fix round 3 of lane launchgates; every gap it leaves is a residual recorded here, not another round, and the itd-65 acceptance criteria stand as written. Round 3 passes a "no longer" in a clause a subordinator opens (until, once, when, whenever, if, unless, after, before, while, whose: "retry until the error no longer appears", "branches whose upstream no longer exists"; iss-2609252045148890), and refuses the "previously … now" form split at a sentence end when one sentence opens with "previously" and the next in the same paragraph opens with "now" ("Previously, the ledger was a flat file. Now it is a folder."; iss-2609252045147575). The subordinators end a clause for the "no longer" reading alone, because "until" and "before" also open the time adverbial the "used to" reading reads through ("until v0.6 the dry-run used to skip the tags"). The residuals, each checked against the gate at the change. Passed although they narrate: "today" as the contrast word ("Previously the record lived in the log; today it lives in the ledger."); "used to" after a bare plural subject or an adverbial outside the lexicon ("Older versions used to print a banner.", "Versions before 0.6 used to write the receipt.", "Back then the tool used to print a banner.", "The dry-run, until v0.6, used to skip the tags."); a subordinator that opens a prepositional phrase or a subject's relative clause rather than the clause holding the "no longer" ("After the upgrade the verb no longer writes a receipt.", "An intent whose criteria the reviewer cannot enumerate can no longer be planned."); and the split form when "previously" does not open the first sentence or a paragraph break falls between the two ("The default was previously JSON. Now it is YAML."). Refused although it states present state: an "-ed" word within two words of a "previously" that modifies a verb ("Reviewers previously assigned are now listed first."), beside the present-state refusals the 2026-09-25 entry above names. The docs-lint present-tense tier and review are the backstop for each passed form, and the docs-lint escape, which every finding names, for each refused one. Survey at the change: 0 findings in the 15 shipped doc bodies; over every Markdown file in the tree the finding locations go from 422 to 410, 12 dropped and none added, every drop a "no longer" after a subordinator in a record, research note, brief chapter or plugin command page, one of them a genuine narration the residual above names (implementer of lane fix3-launchgates).
- 2026-09-25 — Three further residuals of the narration hard tier, found by the round-4 verification and recorded rather than fixed (the last round on the heuristic): a comma-less fronted adverbial clause before a "no longer" main clause passes ("Once you upgrade the verb no longer writes a receipt."); the split "previously ... now" form passes across two list items, since each item is its own run; and a parenthetical between the subordinator and "no longer" is refused though it states present state ("Retry until, after a pause, the error no longer appears."). None occurs in a shipped doc body; the escape marker and the docs-lint present-tense tier are the backstop (orchestrator abcd-39, autonomous run A).
