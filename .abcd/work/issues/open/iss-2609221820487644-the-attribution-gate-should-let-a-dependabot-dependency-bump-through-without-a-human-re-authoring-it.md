...
- 2026-09-22, the product thinker: route 2 of the three shapes above. A workflow re-authors a bot-opened bump as the repository owner BEFORE the gate runs; the gate itself does not change. The bound is the diff's shape (an ecosystem's manifest and its lock file, on a branch the forge marks as that bot's) and it covers every ecosystem dependabot opens, not Go alone. The re-authored message names the bot and the workflow and carries `Assisted-by: None`. It is scaffolded into a managed repository by the same verb that writes its release workflows, opt-in per repository. Named rather than assumed: a workflow asserts the owner's authorship with no person in the loop, which the owner accepts once in the configuration under that bound. Planned as itd-2609221842494980.
