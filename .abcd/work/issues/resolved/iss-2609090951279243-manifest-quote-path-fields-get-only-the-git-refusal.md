...
resolution: "Every quote-type manifest path field is now held to a closed set declared for that field, alongside the .git refusal it used to get alone; the committed manifest still loads and site-render still passes."
impact: fix
---
...
- pursued: closing each quote field to a stated set stops an unreviewed manifest edit reaching the gitignored local tier or a repository dotfile; it would be shown wrong by a legitimate composition source that no per-field set can name, which would surface as a manifest the repository cannot express rather than as a silent leak
