...
resolution: "Took the record's second branch: hasOpaqueSessionID's comment states the coverage boundary (an all-lower-case non-hex id is not detected, since a documentation slug has that shape; a harness minting such ids is covered by naming its host, not by widening the opacity test), and TestHarnessSessionURLCoverageBoundary pins it. The external enumeration of harness id shapes was not performed; it cannot be settled from the repository."
impact: internal
resolved_by:
  commit: "69cc1e4bafd0aa685fa22f8e9fd2e9c4bb41eac1"
---
...
- pursued: the boundary is written where the rule lives and a widening fails a test; a covered harness later shown to mint lower-case non-hex ids would show the stated boundary needs a host-specific rule
