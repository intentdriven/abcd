...
- pursued: we expect absence-is-absence to be the smaller and safer of the record's two options because it adds no configuration surface and leaves every present-manifest failure exactly as it was; it is shown wrong if a repository legitimately needs to declare that it has no plugin, rather than simply not having one
