...
resolution: "spec_lifecycle refuses a planned intent with no open spec beside a closed one, and a shipped intent with a spec still open."
impact: fix
resolved_by:
  commit: "fdb77e5b"
---
...
- pursued: a planned intent with only closed specs and a shipped intent with an open spec are each refused on the intent, while a planned intent with a closed spec and an open remainder passes; a mis-merged pair passing record-lint would show it wrong
