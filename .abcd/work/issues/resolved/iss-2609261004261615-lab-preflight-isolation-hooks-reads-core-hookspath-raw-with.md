...
resolution: "isolation.hooks reads core.hooksPath with git config --type=path --get and judges git's expansion; %(prefix)/, ~user, outside ~/ and empty values halt, an in-lab ~/ value passes (TestPreflightJudgesTheHooksPathGitExpands)."
impact: fix
resolved_by:
  commit: "508afa1f"
---
...
- pursued: a global %(prefix)/ or empty hooks path now halts the preflight naming isolation.hooks; a git whose --type=path expansion differs from the path it runs hooks from would show it wrong
