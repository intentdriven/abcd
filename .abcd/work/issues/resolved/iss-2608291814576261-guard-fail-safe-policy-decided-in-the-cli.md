...
resolution: "guard.LoadRepo returns a typed Loaded result whose posture (clean, repo layer dropped, unavailable) is decided once in core; the hook, the check verb and ahoy's health report format it instead of counting entries."
impact: internal
resolved_by:
...
- pursued: every surface reads one posture from core; shown wrong by the hook and the check disagreeing on one repo state (TestGuardHookAndCheckAgreeOnAnUncommittedKillSwitch)
