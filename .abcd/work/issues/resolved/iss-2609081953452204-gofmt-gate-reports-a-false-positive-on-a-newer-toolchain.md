...
resolution: "The format gate resolves gofmt from the go.mod toolchain via make fmt-check, which CI now invokes instead of restating; it refuses loudly rather than falling back to PATH."
impact: internal
---
...
- pursued: one gate resolved through the declared toolchain cannot disagree with itself across machines, so the false positive on a newer gofmt is gone for good; a fmt-check that still names a file on an unmodified checkout, or a CI job that formats differently from the target, would show it wrong.
