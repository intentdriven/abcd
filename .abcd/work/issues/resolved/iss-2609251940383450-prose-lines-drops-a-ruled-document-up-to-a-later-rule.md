...
resolution: "proseLines drops a leading --- block only when it reads as YAML frontmatter (a key: entry, and otherwise only continuations, list items, comments or blank lines); a ruled document whose block holds prose is read whole (TestProseLinesReadsARuledDocumentWhole)."
impact: fix
resolved_by:
  commit: "68c70ba3"
---
...
- pursued: a document opening with a rule and repeating one later has its prose read by both content gates while real frontmatter stays metadata; a narration or marker finding missing between the two rules, or a finding raised inside YAML frontmatter, would show it wrong
