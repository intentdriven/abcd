...
resolution: "docs lint reports 'nothing was checked' (and checks: 0 in --json) when the config arms no rule, and the scaffolded docs-lint config carries abcd's Writing-Guide present_tense and spelling token families and the links_resolve, harness_leak and stray_root_docs rules armed, with banned names empty, the harness family left to the repository, and the em-dash-in-list-item house-style token seeded at the severity the adopter chooses at install, blocking or warning, and at warning under an unattended install; a docs lint that checked nothing (no rule armed, or roots holding no markdown document) warns loudly on stderr and in --json and still exits 0"
impact: fix
resolved_by:
  commit: "5aaf6f2c"
---
