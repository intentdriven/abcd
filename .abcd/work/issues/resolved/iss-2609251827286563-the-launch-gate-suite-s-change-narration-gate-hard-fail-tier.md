...
resolution: "Each ambiguous narration construct carries a check only its narrating reading passes (past-habit used to; no longer and renamed beside a subject naming abcd or its behaviour; previously/now beside a change verb), the finding names the docs-lint escape, and the reviewer's four sentences and the same-sentence AC10 case pass (TestNarrationGatePassesPresentTense)."
impact: fix
resolved_by:
  commit: "dfdb906d"
---
...
- pursued: the four present-state sentences pass while every existing narration fixture still hard-fails, and a whole-tree survey shows no new finding location; a present-state sentence of those shapes refused, or a fixture in TestNarrationGateHardFailsOnAChangeConstruct passing, would show it wrong
