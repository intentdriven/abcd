...
- pursued: a handle adjacent to a hyphen is part of a longer token, so refusing it closes the boundary the word-boundary rule left open without needing the export's id list; it would be shown wrong by a changelog that credits a record by a handle deliberately written adjacent to a hyphen, which would surface as a missing version stamp rather than as a wrong one
