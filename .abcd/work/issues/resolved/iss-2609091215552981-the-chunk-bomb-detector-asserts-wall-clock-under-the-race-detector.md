...
resolution: "TestPNGChunkCountBombIsBounded asserts the refusal the chunk bound produces rather than the wall clock it produces it in, so a slow machine and a broken bound are no longer the same result; the duration survives as a runaway guard alone, at a ceiling that moves with the race detector through a build-tagged raceEnabled constant."
impact: internal
---
...
- pursued: we expect a bound to be proven by the refusal it emits, so the test reports the property it names on any machine and under any instrumentation; a run that passes with the bound raised, or one that reds on a slow machine where the bound held, would show it wrong.
