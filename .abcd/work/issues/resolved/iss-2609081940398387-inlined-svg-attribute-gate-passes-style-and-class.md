...
resolution: "checkSVGAttr now holds an inlined drawing's attributes to a closed allowlist built from the 32 committed SVGs plus the standard presentation and geometry set, refusing style, class, foreign namespaces and anything unweighed by name"
impact: fix
---
...
- pursued: the inlining gate's attribute half was a denylist in behaviour, so closing it over the corpus's real attribute set stops style/class reaching the page at no cost to the committed drawings; a committed or future drawing refused by the site build would show the enumeration was too narrow
