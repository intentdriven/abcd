...
- `itd-2609111003026787` — Every record family retires a record the same way: a terminal-move surface with one shape and one shared vocabulary across issues, specs, intents and ADRs, including the two supersessions that have no verb today (`refines` adr-45, which made minting uniform and left retirement asymmetric; the stale ADR-minting rule it surfaced is iss-2609111002410678)
<!-- /index -->
