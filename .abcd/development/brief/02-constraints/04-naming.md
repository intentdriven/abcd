...
| `force_refresh_every_n` | `.abcd/config.json` field under the `rules.` namespace. Integer; default 15 (`rules.DefaultRefreshBackstop`), which a missing key or a non-positive value falls back to. Every N prompts, the prompt-router hook forces a full re-inject regardless of dedup signature match (compaction recovery). | spc-14 spec + `spc-14-modular-rules-loader-prompt-router-hook.4` |
