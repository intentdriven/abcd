...
| `LOAD` | Default plugin-bundled domain, the ninth. Recall keywords (burners, stress runs, `nohup`, a load test or load average) trigger injection of the trust rule for load experiments: one owned process group killed together, clean proven by what is running, consent and a cap below the core count on a live development machine. | iss-2609210828122412 + the 2026-09-23 load-experiment entry in `.abcd/work/DECISIONS.md` |
