...
| `/abcd:capture resolve <iss-N> "<resolution-note>" --impact <additive\|breaking\|fix\|internal> --grounds "<token>: <text>"` | Mark issue resolved. `--impact` is required — resolving without it is refused and writes nothing; `--grounds` is recorded when given, its absence refusal parked by iss-2609091009111294; the grounds are the conjecture being acted on, not the route taken. Three optional `resolved_by` provenance flags name what fixed it: `--intent itd-N` (must exist), `--spec spc-N` (must exist), `--commit <sha>` (shape-checked hex; the RS002/RS003 gates check reachability). `--shipped-in vX.Y.Z` is migration use only: it names the release that already carried the work, so the record stays out of the current cut. `--production-mode` restamps how the text was produced, and is refused on a record that predates disclosure | `open/` → `resolved/` |
...
lapsed_at: <rfc3339-utc>   # optional, never defaulted (absence refusal parked, iss-2609091009111294); on a lapse: the instant the discipline gave way, not the write-up
