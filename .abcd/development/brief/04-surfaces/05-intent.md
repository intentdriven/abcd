...
3. Spec marked done in the native spec store   (standalone + bundle: work complete)
   ├─ a MANUAL step, run in the same change that lands the work: closing the spec with the `spec` verb (CLI-only; no hook or
   │  gate runs it, and a planned intent whose code is on main is invisible to the launch cut, which composes
