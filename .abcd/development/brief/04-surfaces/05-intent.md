...
- For intents promoted from issues (per itd-4): bidirectional `related_issues` ↔ `related_intents` linkage holds. Checked by `abcd intent audit --issue-drift` (the intent-auditor's issue-drift role, in the predecessor store's spc-23 shape), not by the record-lint.
