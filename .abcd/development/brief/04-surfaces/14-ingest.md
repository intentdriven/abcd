...
hand than a bare fetch would. The `abcd-ingest <url-or-file>` human quick path
that `commands/ingest.md` mentions is **not a demonstrable surface**: no binary
sub-verb and no repo-shipped script provides it, so where it exists at all it is
an operator-local convenience outside the corpus contract.
