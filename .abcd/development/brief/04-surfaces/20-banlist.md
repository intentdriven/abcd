...
| public family | `.abcd/docs-lint.json` | only where git says the path would be tracked. Seeded with an **empty** entry list: abcd cannot know which names a repo may not publish, and a ban nobody declared would fail a build over a word the maintainer never chose |
