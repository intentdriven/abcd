...
- Stored text carrying a secret or identity span the write-time store redactor would refuse or rewrite blocks at lint (lint code `MR001`) — pages, `.sources_index.json` and text kept-originals alike. Lint names the kind and the line, never the span, and never rewrites the store; the operator repairs.
