...
> **`abcd implement <itd-N>` runs one intent from READY to delivered with nobody in the loop, and refuses to start while a design decision is still open.** The verb is a loop over a state file, not a model holding the run in its head: every invocation reads the state, does one step, writes the state and exits, so a killed process loses nothing and a pause is a timestamp. A host session drives it step by step by default, and a process can drive the same loop through a configured runner. Two modes are explicit: `abcd intent plan` stays the human's act, and `implement --auto-plan` is the agent's, adversarial reviews included, ruled on in its own decision record before it ships.
>
...
- **Given** a READY intent nobody holds, **when** `abcd implement next` runs, **then** the state file exists with one lane, the worktree exists in abcd's form on a branch off the default branch, and the brief file names the intent, the spec and the conventions it was rendered from.
...
- **Given** a host with no configured runner, **when** `abcd implement next` returns a brief, **then** the host is told which agent to start and where the receipt goes, and the loop advances only on `abcd implement receipt`.
