...
- **Acceptance criterion 1 (the missing half).** Every surface file under `04-surfaces/` carries a sub-verb table. At closure nine files carry no `## Sub-verbs` table: `08-abcd`, `09-reflect`, `12-version`, `13-consult`, `14-ingest`, `15-prepare-this-repo`, `21-update`, `24-decide` and `29-report` (the last three were added after spc-27 closed).
- **Acceptance criterion 4 (the missing half).** Host-delegated surfaces (`consult`, `ingest`, `prepare-this-repo`) carry rows, exempt from the cobra check only. At closure the exemption config exists (the `surface_coverage` rule's `host_delegated` list in `.abcd/record-lint.json`) but those surfaces have no table at all. Their command pages define no sub-verb, so the table each carries is header-only: a row would claim a verb that exists nowhere, and the rows the criterion exempts from the cobra check are whatever rows the table holds, format-checked still.
